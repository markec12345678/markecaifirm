    ...
    if (lastMonthProfit > 0 && thisMonthProfit > 0) {
      ...
      if (profitChange <= -20) {
        anomalies.push({
          ...
          recommendation: 'Analiziraj kaj se je spremenilo — manj prodaj, nižji ROI, ali več fees?',
      ...
      if (lastMonth && lastMonth.profit > 0 && thisMonth.profit < lastMonth.profit) {
        ...
        if (drop >= 30) {
          anomalies.push({
            ...
            description: `ROI v kategoriji ${cat} je padel za ${Math.round(drop)}% (z ${Math.round(lastMonth.profit)}€ na ${Math.round(thisMonth.profit)}€).`,
            metric: `Profit ${cat}`,
    ...
    if (categoryRanking.length > 0) {
      ...
      if (heldInTop === 0) {
        predictions.push({
          ...
          description: `Kategorija "${top.category}" ima ${top.roi}% ROI in ${top.count} prodaj, ampak 0 held. Primerna za ponovni nakup.`,
          metric: 'ROI',
