// v6.52 / v8.96.3-batch1: AI Buyer Behavior Predictor — napove naslednji nakup kupca in behavioral pattern
...
export const POST = withAiRoute<BuyerBehaviorInput>({
  endpoint: '/api/ai/buyer-behavior-predictor',
  maxDuration: 90,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, predictor: null, message: 'Ni prodaj za behavior analizo.' });
    }
    ...
    const buyers = computeBehaviorMetrics(Array.from(buyerMap.values()), now);
...
function computeBehaviorMetrics(buyers: BuyerBehavior[], now: number): BuyerBehavior[] {
  return buyers.map(b => {
...
function buildPrompt(targetBuyers: BuyerBehavior[]): string {
  ...
  return `Si AI buyer behavior predictor za slovenske oglasne platforme.
...
function transformPredictor(parsed: any, targetBuyers: BuyerBehavior[]) {
