// v6.71 / v8.95.7-listing: AI Listing Multi-Variant Tester — A/B/n testing z ML in statistical significance
...
// Returns: { ok, tester: { listings, variants, statistics, winners, mlPredictions, summary } | null }
...
interface ListingMultiVariantTesterInput {
  tradeId: string | null;
  numVariants: number;
}

export const POST = withAiRoute<ListingMultiVariantTesterInput>({
  endpoint: '/api/ai/listing-multi-variant-tester',
  maxDuration: 120,
  enforceBudget: true,
  ...
  parseBody: async (req) => {
    ...
    return {
      ...
      numVariants: Math.max(2, Math.min(5, Number(body?.variants ?? 4))),
    };
  },

  // No validateInput — tradeId je opcijski, numVariants ima default + clamp

  handler: async (input, ctx: AiRouteContext) => {
    const { db, callAi, parseAi } = ctx;
    const { tradeId, numVariants } = input;
    ...
    const heldTrades = await db.trade.findMany({
      ...
      select: {
        ...
        listing: { select: { aiEstimatedValue: true, description: true, detailDescription: true, price: true, imageUrl: true, location: true } },
      },
      take: tradeId ? 1 : 8,
    });

    if (heldTrades.length === 0) {
      return apiOk({ ok: true, tester: null, message: 'Ni held tradeov za multi-variant testing.' });
    }

    const items = buildItems(heldTrades);
    const itemsStr = buildItemsStr(items);
    const prompt = buildPrompt(numVariants, items.length, itemsStr);

    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);

    const tester = transformTester(parsed, items, numVariants);

    return apiOk({ ok: true, tester });
  },
});
...
function buildItems(heldTrades: HeldTradeRow[]): VariantItem[] {
  return heldTrades.map(t => ({
    ...
    price: t.listing?.price ?? Math.round((t.buyPrice + (t.buyFees ?? 0)) * 1.25),
  }));
}

function buildItemsStr(items: VariantItem[]): string {
  return items
    .slice(0, 8)
    .map(i => `- [${i.id}] "${i.title}" | ${i.category} | ${i.price}€`)
    .join('\n');
}

function buildPrompt(numVariants: number, itemsCount: number, itemsStr: string): string {
  return `Si AI listing multi-variant tester z ML in statistical significance testing.
...
function transformTester(parsed: any, items: VariantItem[], numVariants: number): any {
