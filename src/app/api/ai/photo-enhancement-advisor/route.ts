// v7.61: AI Photo Enhancement Advisor — za HELD inventar s slikami (imageUrl)
...
function deterministicAdvice(item: HeldItemData): PhotoAdviceItem {
  ...
  return {
    ...
    estimatedPriceUplift: clampUplift(null, item.aiEstimatedValue, Math.round(item.aiEstimatedValue * 0.08)),
    overallAdvice: `Sedanja foto kakovost je zmerna (${photoScore}/100). Z boljšo osvetlitvijo, čistim ozadjem in dodatnimi koti lahko skrajšaš čas prodaje za ~7 dni in dvigneš ceno za ~${clampUplift(null, item.aiEstimatedValue, Math.round(item.aiEstimatedValue * 0.08))}€.`,
  };
}
...
export async function GET(req: NextRequest) {
  return handlePhotoEnhancement(req);
}
export async function POST(req: NextRequest) {
  return handlePhotoEnhancement(req);
}

async function handlePhotoEnhancement(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, 'ai-photo-enhancement-advisor', 20);
    ...
    const prompt = `Si profesionalni fotograf in e-commerce vizualni svetovalec za slovenske oglasne platforme (Bolha, Vinted, mobile.de, Kleinanzeigen).
...
5. estimatedPriceUplift: EUR — za koliko se bo dvignila cena (0 do ${Math.round(items[0].aiEstimatedValue * 0.15)}€, max estValue × 0.15)
...
Odgovori LE z JSON:
{
  "items": [
    ...
    try {
      ...
      const parsed = parseJsonLooseExported(raw) as AiAdvisorResponse | null;
    ...
    const summary: PhotoAdviceSummary = {
    ...
    logger.error('/api/ai/photo-enhancement-advisor', 'handler failed', err);
