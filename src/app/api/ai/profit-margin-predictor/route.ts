 ...
 */
// v6.47 / v8.96.2-batch3: AI Profit Margin Predictor — pred-nakupna ocena dobička za potencialne investicije
...
export const POST = withAiRoute<ProfitMarginPredictorInput>({
  endpoint: '/api/ai/profit-margin-predictor',
  maxDuration: 90,
  ...
  parseBody: async (req) => {
    ...
    return { listingId, listing, budget };
  },

  // No validateInput — listingId OR listing OR neither (uses PRILIKA fallback)

  handler: async (input, ctx: AiRouteContext) => {
    const { db, callAi, parseAi } = ctx;
    const { listingId, listing: listingInput, budget: userBudget } = input;

    // logDeprecatedCall PRESERVED — Phase 2 deprecation logging (ctx.req uporabljen namesto req parametra)
    logDeprecatedCall('/api/ai/profit-margin-predictor', ctx.req, '/api/ai/profit-margin-predictor-v3');

    // 1. Pridobi target listings (iz listingId, listingInput ali PRILIKA fallback)
    const targetListings = await resolveTargetListings(db, listingId, listingInput);
    ...
    const prompt = buildPrompt(targetListings, userBudget);
...
function buildPrompt(targetListings: TargetListing[], userBudget: number): string {
  ...
  return `Si AI profit margin predictor za slovenske oglasne platforme.
