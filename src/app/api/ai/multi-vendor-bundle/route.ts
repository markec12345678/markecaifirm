// v6.16 / v8.96.0-batch3: AI Multi-Vendor Bundle Deals — AI kombinira inventar iz različnih virov/prodajalcev
...
interface MultiVendorBundleInput {
  maxItems: number;
}

export const POST = withAiRoute<MultiVendorBundleInput>({
  endpoint: '/api/ai/multi-vendor-bundle',
  maxDuration: 90,
  enforceBudget: true, // AI klic — preveri budget

  parseBody: async (req) => {
    const body = await req.json().catch(() => ({}));
    return { maxItems: Math.max(3, Math.min(15, Number(body?.maxItems) || 8)) };
  },

  // No validateInput — maxItems ima clamp default
...
function buildPrompt(d: PromptData): string {
  return `Si ekspert za multi-vendor bundle strategije pri preprodaji.
Kombiniraj iteme iz RAZLIČNIH virov/prodajalcev v bundle za maksimalni dobiček.

INVENTAR PO VIRIH (${d.sourcesCount} virov: ${d.sourcesJoined}):
${d.itemsStr}
