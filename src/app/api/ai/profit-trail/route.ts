// v6.35 / v8.96.0-batch2: AI Profit Trail Visualizer — vizualizira dobičkovno pot vsakega itema
...
interface ProfitTrailInput {}

export const POST = withAiRoute<ProfitTrailInput>({
  endpoint: '/api/ai/profit-trail',
  maxDuration: 90,
  enforceBudget: true, // AI klic — preveri budget

  parseBody: async (req) => {
    await req.json().catch(() => ({}));
    return {} as ProfitTrailInput;
  },
  ...
  handler: async (_input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, trails: [], message: 'Ni prodaj za profit trail.' });
    }
    ...
    const prompt = buildPrompt({ trailsStr, heldStr });
    ...
    const trails = transformTrails(parsed, validSoldIds, validHeldIds);

    return apiOk({ ok: true, trails });
  },
});
...
function buildPrompt(d: PromptData): string {
  return `Si ekspert za vizualizacijo dobičkovne poti (profit trail) pri preprodaji.
...
function transformTrails(parsed: any, validSoldIds: Set<string>, validHeldIds: Set<string>) {
  return {
