// v6.0: AI Trend Predictions — AI napove tržne trende za kategorije
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Group by "category" (extract from userNotes or title keywords)
    const categoryMap: Record<string, any[]> = {};
    for (const l of listings) {
      const cat = extractCategory(l.title, l.monitor?.source);
    ...
    if (Object.keys(categoryMap).length === 0) {
      return NextResponse.json({ ok: true, predictions: [], message: 'Ni dovolj podatkov za napoved.' });
    }

    // Calculate stats per category
    const categoryStats = Object.entries(categoryMap).map(([cat, items]) => {
      ...
      return {
        ...
        sources: Array.from(new Set(items.map(l => l.monitor?.source).filter(Boolean))),
      };
    }).filter(s => s.count >= 3); // Need at least 3 listings

    if (categoryStats.length === 0) {
      return NextResponse.json({ ok: true, predictions: [], message: 'Premalo podatkov (manj kot 3 na kategorijo).' });
    }
    ...
    logger.error("/api/ai/trend-predictions", "POST handler failed", e);
    return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 });
  }
}

function buildTrendPrompt(stats: any[], days: number): string {
  const lines: string[] = [
