// v6.86 / v8.95.3-batch2: AI Buyer Loyalty Predictor v2 — ML napoved loyalnosti kupcev z behavior prediction
...
// Returns: { ok, predictor: { overview, buyers, loyaltyFactors, predictions, interventions, mlModels, summary } | null, message? }
...
interface BuyerLoyaltyPredictorV2Input {
...
export const POST = withAiRoute<BuyerLoyaltyPredictorV2Input>({
  endpoint: '/api/ai/buyer-loyalty-predictor-v2',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, predictor: null, message: 'Ni prodaj za loyalty prediction.' });
    }
...
function buildPrompt(targetBuyers: BuyerRow[]): string {
  ...
  return `Si AI buyer loyalty predictor v2 z ML in behavior prediction.
