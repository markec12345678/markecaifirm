// v6.73 / v8.95.7-listing: AI Listing Conversion Funnel Optimizer — optimizira conversion funnel z ML in drop-off analysis
...
// Returns: { ok, optimizer: { funnel, dropoffs, optimizations, experiments, mlModels, summary } | null }
...
interface ListingConversionFunnelOptimizerInput {
  tradeId: string | null;
  days: number;
}

export const POST = withAiRoute<ListingConversionFunnelOptimizerInput>({
  endpoint: '/api/ai/listing-conversion-funnel-optimizer',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    const heldTrades = await db.trade.findMany({
      ...
      select: {
        ...
        listing: { select: { aiEstimatedValue: true, dealScore: true, contactStatus: true } },
      },
      take: tradeId ? 1 : 20,
    });
    const contactedListings = await db.listing.findMany({
      where: { contactStatus: { not: 'none' }, firstSeenAt: { gte: since } },
      select: { id: true, title: true, contactStatus: true, contactedAt: true, sellerResponse: true },
      take: 300,
    });

    if (soldTrades.length === 0 && heldTrades.length === 0) {
      return apiOk({ ok: true, optimizer: null, message: 'Ni podatkov za funnel optimizacijo.' });
    }

    const prompt = buildPrompt(days, soldTrades.length, heldTrades.length, contactedListings.length);
...
function buildPrompt(days: number, soldCount: number, heldCount: number, contactedCount: number): string {
  return `Si AI listing conversion funnel optimizer z ML in drop-off analysis.
Optimizira conversion funnel z 10 fazami za zadnje ${days} dni.

STATS:
- Prodano: ${soldCount}
- Held: ${heldCount}
- Kontaktiranih listingov: ${contactedCount}
...
function transformOptimizer(parsed: any): any {
  return {
