// v8.10: AI Profit Per Trade Growth Maximizer — AI MAKSIMIZIRA GROWTH profit-a
...
export async function GET(req: NextRequest) {
  return handleProfitPerTradeGrowthMaximizer(req);
}
export async function POST(req: NextRequest) {
  return handleProfitPerTradeGrowthMaximizer(req);
}

async function handleProfitPerTradeGrowthMaximizer(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, 'ai-profit-per-trade-growth-maximizer', 20);
