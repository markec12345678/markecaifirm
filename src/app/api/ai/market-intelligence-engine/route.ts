// v7.76: AI Market Intelligence Engine — AI-powered celovit "executive
...
export async function GET(req: NextRequest) {
  return handleMarketIntelligence(req);
}
export async function POST(req: NextRequest) {
  return handleMarketIntelligence(req);
}

async function handleMarketIntelligence(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, 'ai-market-intelligence-engine', 20);
    ...
    if (listings.length === 0) {
      return NextResponse.json({
        ...
        strategicRecommendation: {
          action: 'MAINTAIN' as StrategicAction,
    ...
    for (const l of listings) {
      // Use monitor.source as the categorization dimension (Listing has no `category`)
    ...
    // 3) Compute 6 signals per source/category (all 0-100).
    // NOTE: Listings are grouped by monitor.source (Bolha / Vinted / etc.)
    // because Listing has no `category` field. We use `category` as the
    // response field name for backward-compat with the spec, but values
    // are platform source names. Trade.category is a separate dimension
    // and is NOT mixed in here to avoid confusion.

    // 4) Compute 6 signals per category (all 0-100)
    const categoryIntelligenceBaseline: CategoryIntelligence[] = [];
    ...
    for (const [cat, agg] of catMap.entries()) {
      ...
      categoryIntelligenceBaseline.push({
        category: cat,
        sentimentScore,
        depthScore,
        saturationScore,
    ...
    // 5) Compute deterministic key findings, opportunities, threats
    ...
    const deterministicOpportunities: Opportunity[] = topCategories
      ...
      .slice(0, 3)
      .map((c) => {
        // Estimate expected profit from market signals (heuristic, no historical)
    ...
    // Deterministic strategic recommendation
    const confidenceLevel = Math.round(
      Math.max(0, Math.min(100, 40 + topCategories.length * 4 + (listings.length / 1000) * 20)),
    );
    ...
    const prompt = `Si AI "Market Intelligence Engine" za slovenske in srednjeevropske oglasne platforme (Bolha, Vinted, Avtonet, mobile.de).
...
VRNI LE JSON:
{
  ...
  "summary": "..."
    ...
    let marketOverview = deterministicOverview;
    let keyFindings = deterministicFindings;
    let opportunities = deterministicOpportunities;
    let threats = deterministicThreats;
    let categoryIntelligence = topCategories;
    let strategicRecommendation = deterministicStrategic;
    let summary = deterministicOverview;
    let aiUsed = false;
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        // marketOverview
        if (typeof parsed.marketOverview === 'string' && parsed.marketOverview.trim().length > 0) {
          marketOverview = clampString(parsed.marketOverview, 300, deterministicOverview);
        }

        // keyFindings (max 5, all clamped)
        if (Array.isArray(parsed.keyFindings)) {
          const newFindings: KeyFinding[] = [];
          for (const f of parsed.keyFindings.slice(0, 5)) {
        ...
        if (Array.isArray(parsed.opportunities)) {
          ...
          for (const o of parsed.opportunities.slice(0, 3)) {
        ...
        if (Array.isArray(parsed.threats)) {
          ...
          for (const t of parsed.threats.slice(0, 3)) {
        ...
        if (Array.isArray(parsed.categoryIntelligence)) {
          const newCatIntel: CategoryIntelligence[] = [];
          for (const c of parsed.categoryIntelligence.slice(0, 15)) {
        ...
        // strategicRecommendation
        if (parsed.strategicRecommendation && typeof parsed.strategicRecommendation === 'object') {
          const sr = parsed.strategicRecommendation as Record<string, unknown>;
          const action = clampEnum(sr.action, VALID_ACTION, deterministicAction);
          const reasoning = clampString(sr.reasoning, 300, deterministicStrategic.reasoning);
          const confLevel = clampNumber(sr.confidenceLevel, 0, 100, confidenceLevel);
          strategicRecommendation = {
            action,
            reasoning,
        ...
        if (typeof parsed.summary === 'string' && parsed.summary.trim().length > 0) {
          summary = clampString(parsed.summary, 500, deterministicOverview);
        }
    ...
    logger.error('/api/ai/market-intelligence-engine', 'handler failed', err);
    return NextResponse.json(
