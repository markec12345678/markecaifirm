...
export async function GET(req: NextRequest) {
  return handleDealSourceVolumeGrowthMaximizer(req);
}
export async function POST(req: NextRequest) {
  return handleDealSourceVolumeGrowthMaximizer(req);
}

async function handleDealSourceVolumeGrowthMaximizer(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, 'ai-deal-source-volume-growth-maximizer', 20);
