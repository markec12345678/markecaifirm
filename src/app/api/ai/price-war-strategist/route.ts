// v6.49: AI Price War Strategist — defensive/offensive strategije za price war s competitorji
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (recentListings.length === 0 && heldTrades.length === 0) {
      return NextResponse.json({ ok: true, strategist: null, message: 'Ni dovolj podatkov za price war analizo.' });
    }

    // 4. Agregacija po kategorijah
    ...
    for (const d of droppedListings) {
      const cat = (d.title || '').toLowerCase();
      // Najdi kategorijo preko matching z recentListings (po naslovu)
    ...
    const prompt = `Si AI price war strategist za slovenske oglasne platforme.
...
Odgovori LE z JSON:
{
  ...
  "summary": {
    ...
    const strategist = {
      ...
      wars: (parsed?.wars || [])
        ...
        .slice(0, 8)
        .map((w: any) => {
          const orig = topWars.find(x => x.category === String(w?.category));
          return {
  ...
  } catch (e: any) { logger.error("/api/ai/price-war-strategist", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
