// v6.62: AI Listing Title Generator v2 — ML naslovi z A/B testing in platform optimization
...
export async function POST(req: NextRequest) {
  try {
    ...
    let targetListings: Array<{
      ...
      description: string; estValue: number;
    }> = [];
    ...
    if (targetListings.length === 0) {
      return NextResponse.json({ ok: true, generator: null, message: 'Ni listingov za title generacijo.' });
    }
    ...
    const prompt = `Si AI listing title generator v2 z ML scoring in A/B testing.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/listing-title-generator-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
