// v6.5 / v8.95.6-other: Seasonal Trend Calendar — AI zazna sezonske vzorce v cenah
...
export const GET = withAiRoute<Input>({
  endpoint: '/api/ai/seasonal-calendar',
  maxDuration: 90,
  ...
  handler: async (_input, ctx: AiRouteContext) => {
    ...
    if (listings.length < 30) {
      return apiOk({ ok: true, calendar: [], insights: [], message: 'Premalo podatkov (potrebno vsaj 30 oglasov v 2 letih).' });
    }

    // Group by month
    const months = buildMonthlyData(listings);

    // Calculate seasonal patterns (year-over-year comparison for same months)
    const seasonalPatterns = buildSeasonalPatterns(months);
    ...
    // Generate insights
    const insights = buildInsights(bestBuyMonth, bestSellMonth, seasonalPatterns);
...
function buildMonthlyData(listings: ListingRow[]): MonthData[] {
  // Group by month
  ...
  for (const l of listings) {
    ...
    // Original used `l.price!` because where clause `price: { not: null }` filters
    // nulls at runtime (TypeScript doesn't deduce runtime filtering)
...
function buildSeasonalPatterns(months: MonthData[]): SeasonalPattern[] {
  // Calculate year-over-year comparison for same months
  const byMonthNum: Record<number, MonthData[]> = {};
...
function buildInsights(
  bestBuyMonth: SeasonalPattern | undefined,
  bestSellMonth: SeasonalPattern | undefined,
