// v6.66 / v8.95.4-batch4: AI Buyer Win-Back Campaign Designer — oblikuje win-back kampanje z ML in multi-touch
...
export const POST = withAiRoute<BuyerWinBackInput>({
  endpoint: '/api/ai/buyer-win-back-campaign-designer',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, designer: null, message: 'Ni prodaj za win-back campaign design.' });
    }
    ...
    const prompt = buildPrompt({ dormant30, dormant60, dormant90, churned, highValueLost });
    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);

    const designer = transformDesigner(parsed, { dormant30, dormant60, dormant90, churned, highValueLost, monthsAhead });
...
function buildBuyers(soldTrades: SoldTradeRow[]): BuyerRow[] {
  ...
  return Array.from(buyerMap.values()).map(b => { b.daysSinceLast = b.lastPurchase ? Math.round((now - b.lastPurchase.getTime()) / (24*60*60*1000)) : 999; return b; });
}
...
interface StatsContext {
  ...
  dormant90: number;
  churned: number;
  highValueLost: number;
}

function buildPrompt(ctx: StatsContext): string {
  const { dormant30, dormant60, dormant90, churned, highValueLost } = ctx;
  return `Si AI buyer win-back campaign designer z ML in multi-touch strategijo.
...
function transformDesigner(parsed: any, ctx: StatsContext & { monthsAhead: number }): any {
  const { dormant30, dormant60, dormant90, churned, monthsAhead } = ctx;
  return {
