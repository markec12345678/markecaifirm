...
export async function GET(req: NextRequest) {
  return handleProfitPerEuroMaximizer(req);
}
export async function POST(req: NextRequest) {
  return handleProfitPerEuroMaximizer(req);
}

async function handleProfitPerEuroMaximizer(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, 'ai-profit-per-euro-maximizer', 20);
