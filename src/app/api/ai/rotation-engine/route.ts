// v6.38 / v8.95.8-refactor: AI Inventory Rotation Engine — optimizira rotacijo inventarja za max cash flow
...
export const POST = withAiRoute<RotationEngineInput>({
  endpoint: '/api/ai/rotation-engine',
  maxDuration: 90,
  ...
  handler: async (_input, ctx: AiRouteContext) => {
    ...
    if (heldTrades.length === 0) {
      return apiOk({ ok: true, rotation: null, message: 'Ni held tradeov za rotation engine.' });
    }
    ...
    const prompt = buildPrompt({
      ...
      totalHeldValue: Math.round(totalHeldValue),
    ...
    const rotation = transformRotation(parsed, items, totalHeldValue);

    return apiOk({ ok: true, rotation });
  },
});
...
function buildPrompt(d: PromptData): string {
  return `Si AI inventory rotation engine za optimizacijo obrtnosti inventarja.
Cilj: maksimizirati cash flow z optimalno rotacijo (prodaja + reinvestiranje).

TRENUTNO STANJE:
- Held: ${d.heldCount} itemov (${d.totalHeldValue}€)
- Realizirano: ${d.totalRealized}€
- Povp. dni do prodaje: ${d.avgDaysToSell}
- Turnover ratio: ${d.currentTurnover.toFixed(2)}
...
function transformRotation(parsed: any, items: RotationItem[], totalHeldValue: number): {
  insights: string;
  currentCycle: any;
  items: any[];
  rotationPlan: any[];
