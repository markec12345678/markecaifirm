// v6.63 / v8.96.2-batch3: AI Listing Description Generator v3 — ML opisi z personalization in sentiment optimization
...
interface ListingDescriptionGeneratorV3Input {
  tradeId: string | null;
  listingId: string | null;
  targetPersona: string | null;
}

export const POST = withAiRoute<ListingDescriptionGeneratorV3Input>({
  endpoint: '/api/ai/listing-description-generator-v3',
  maxDuration: 120,
  ...
  parseBody: async (req) => {
    ...
    return { tradeId, listingId, targetPersona };
  },
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    const { tradeId, listingId, targetPersona } = input;

    // 1. Pridobi target listings (iz tradeId, listingId ali held fallback)
    const targetListings = await resolveTargetListings(db, tradeId, listingId);
    if (tradeId && targetListings.length === 0) {
      throw new ApiRouteError('Trade ne obstaja', 404);
    }
    if (listingId && targetListings.length === 0) {
      throw new ApiRouteError('Listing ne obstaja', 404);
    }
    if (targetListings.length === 0) {
      return apiOk({ ok: true, generator: null, message: 'Ni listingov za description generacijo.' });
    }

    const prompt = buildPrompt(targetListings, targetPersona);
...
function buildPrompt(targetListings: TargetListing[], targetPersona: string | null): string {
  const itemsStr = targetListings.slice(0, 8).map(i =>
    `- [${i.id}] "${i.title}" | ${i.category} | ${i.price}€ | ${i.location} | opis: ${i.description.slice(0, 100)}`
  ).join('\n');

  return `Si AI listing description generator v3 z ML personalization in sentiment optimization.
...
function transformGenerator(parsed: any, targetListings: TargetListing[]): {
  insights: string;
  listings: any[];
  descriptions: any[];
  mlScoring: any[];
