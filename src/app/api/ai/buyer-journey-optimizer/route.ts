// v6.70 / v8.94-refactor: AI Buyer Journey Optimizer — optimizira buyer journey z ML in touchpoint mapping
...
interface BuyerJourneyOptimizerInput {
...
export const POST = withAiRoute<BuyerJourneyOptimizerInput>({
  endpoint: '/api/ai/buyer-journey-optimizer',
  maxDuration: 90,
  enforceBudget: true,

  parseBody: async (req) => {
    const body = await req.json().catch(() => ({}));
    const raw = body?.customerName ? String(body.customerName).trim() : '';
    return { customerName: raw || null };
  },
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    // 1. Pridobi prodaje (status: sold, s sellLocation in sellDate)
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, optimizer: null, message: 'Ni prodaj za journey optimization.' });
    }

    // 2. Zgradi buyer map iz prodaj
    const buyers = buildBuyers(soldTrades, Date.now());

    // 3. Filtriraj po customerName če podan
    if (customerName) {
    ...
    // 4. Izberi target buyere (filtrirani ali top 20)
...
/**
 * Zgradi seznam buyerjev iz prodaj. Za vsako sellLocation (ime kupca)
 * aggregira število nakupov, skupno porabo, zadnji nakup in kategorije.
 */
function buildBuyers(soldTrades: SoldTradeRow[], now: number): BuyerInfo[] {
  ...
  for (const t of soldTrades) {
    ...
    if (!buyerMap.has(name)) {
      buyerMap.set(name, {
        ...
        daysSinceLast: 0, lastPurchase: t.sellDate, categories: new Set(),
      });
    }
...
/**
 * Zgradi AI prompt za buyer journey optimization. Vsebuje 8 journey faz
 * in JSON shemo z buyers/journeys/touchpoints/optimizations/mlModels/summary.
 */
function buildPrompt(targetBuyers: BuyerInfo[]): string {
  ...
  return `Si AI buyer journey optimizer z ML in touchpoint mapping.
...
/**
 * Transformiraj AI parsed JSON v tipiziran optimizer objekt z clamp/slice
 * validacijo. Filtrira buyers po validNames (samo kupci iz DB query-ja).
 */
function transformOptimizer(parsed: any, targetBuyers: BuyerInfo[]): any {
  ...
  return {
    ...
    journeys: (parsed?.journeys || []).slice(0, 8).map((j: any) => ({
      ...
      buyerCount: Math.max(0, Number(j?.buyer_count ?? 0)),
      avgTimeInStageDays: Math.max(0, Number(j?.avg_time_in_stage_days ?? 0)),
    ...
    summary: {
      ...
      avgJourneyProgressPct: Math.max(0, Math.min(100, Number(parsed?.summary?.avg_journey_progress_pct ?? 50))),
