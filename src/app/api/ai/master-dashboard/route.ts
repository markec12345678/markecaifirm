// v6.40 MILESTONE: AI Master Dashboard — unified view vseh 160+ AI funkcij v eno
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (heldTrades.length === 0 && soldTrades.length === 0) {
      return NextResponse.json({ ok: true, master: null, message: 'Ni podatkov za master dashboard.' });
    }
    ...
    const prompt = `Si vrhovni AI poslovni svetovalec — MASTER DASHBOARD za vso preprodajno operacijo.
...
- Stalled (>30d): ${stalled}
- High risk: ${highRisk}
- Nove priložnosti (7d): ${opportunities}
...
Odgovori LE z JSON:
{
  ...
  "master_summary": "<max 300 znakov — one paragraph executive summary>"
    ...
    const master = {
      ...
      financial: {
        realizedProfitEur: Math.round(Number(parsed?.financial?.realized_profit_eur ?? totalRealized)),
        unrealizedProfitEur: Math.round(Number(parsed?.financial?.unrealized_profit_eur ?? 0)),
        totalRevenueEur: Math.round(Number(parsed?.financial?.total_revenue_eur ?? totalRevenue)),
        avgRoiPct: Math.round(Number(parsed?.financial?.avg_roi_pct ?? avgRoi)),
        avgMarginPct: Math.round(Number(parsed?.financial?.avg_margin_pct ?? 0)),
        cashAvailableEur: Math.round(Number(parsed?.financial?.cash_available_eur ?? currentCash)),
        investedEur: Math.round(Number(parsed?.financial?.invested_eur ?? totalInvestedHeld)),
      ...
      inventory: {
        ...
        totalValueEur: Math.round(totalInvestedHeld),
      ...
      market: {
        ...
        opportunityRatePct: recentListings.length > 0 ? Math.round((opportunities / recentListings.length) * 100) : 0,
    ...
    return NextResponse.json({ ok: true, master, version: 'v6.40.0 MILESTONE' });
  } catch (e: any) { logger.error("/api/ai/master-dashboard", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
