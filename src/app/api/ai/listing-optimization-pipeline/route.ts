// v6.57: AI Listing Optimization Pipeline — celovit pipeline za optimization oglasa (title→desc→price→images→timing)
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (heldTrades.length === 0) {
      return NextResponse.json({ ok: true, pipeline: null, message: 'Ni held tradeov za optimization pipeline.' });
    }
    ...
    const prompt = `Si AI listing optimization pipeline za slovenske oglasne platforme.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/listing-optimization-pipeline", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
