// v6.60 / v8.96.2-batch4: AI Listing SEO Optimizer v2 — advanced SEO z keyword research, competitor analysis in ML ranking
...
export const POST = withAiRoute<ListingSeoOptimizerInput>({
  endpoint: '/api/ai/listing-seo-optimizer-v2',
  maxDuration: 120,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (targetListings.length === 0) {
      return apiOk({ ok: true, optimizer: null, message: 'Ni listingov za SEO optimizacijo.' });
    }

    const prompt = buildPrompt(targetListings, platforms);
    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);
    const optimizer = transformOptimizer(parsed, targetListings, platforms);
...
function buildPrompt(targetListings: TargetListing[], platforms: string[]): string {
  ...
  return `Si AI listing SEO optimizer v2 z keyword research, competitor analysis in ML ranking.
...
function transformOptimizer(parsed: any, targetListings: TargetListing[], platforms: string[]): {
  insights: string;
  listings: any[];
  keywordResearch: any[];
