// v6.80 / v8.95.8-listing: AI Listing Seasonality Optimizer — ML optimizacija oglasov glede na sezonskost
...
// Returns: { ok, optimizer: { listing, seasonalityProfile, peakWindows, recommendations, mlModels, summary } | null }
...
export const POST = withAiRoute<SeasonalityInput>({
  endpoint: '/api/ai/listing-seasonality-optimizer',
  maxDuration: 90,
  enforceBudget: true,
  ...
  parseBody: async (req) => {
    ...
    return {
      ...
      monthsAhead: Math.max(1, Math.min(12, Number(body?.monthsAhead ?? 3))),
    };
  },

  handler: async (input, ctx: AiRouteContext) => {
    const { db, callAi, parseAi } = ctx;
    const { tradeId, monthsAhead } = input;

    const heldTrades = await db.trade.findMany({
      where: { status: 'held' },
      select: { id: true, title: true, category: true, buyPrice: true, buyDate: true, buyLocation: true, notes: true, listingId: true },
      take: 200, orderBy: { buyDate: 'desc' },
    });
    if (heldTrades.length === 0) {
      return apiOk({ ok: true, optimizer: null, message: 'Ni aktivnih oglasov za sezonsko analizo.' });
    }
    ...
    const soldTrades = await db.trade.findMany({
      ...
      select: { id: true, title: true, category: true, sellPrice: true, sellDate: true },
      take: 500, orderBy: { sellDate: 'desc' },
    });
    const now = new Date();
    const currentMonth = now.getMonth() + 1;

    const prompt = buildPrompt({ target, suggestedPrice, soldTrades, currentMonth, monthsAhead });

    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);

    const optimizer = transformOptimizer(parsed, target, suggestedPrice);
...
function buildPrompt(d: PromptData): string {
  const { target, suggestedPrice, soldTrades, currentMonth, monthsAhead } = d;
  ...
  return `Si AI listing seasonality optimizer z ML in time series forecasting.
...
function transformOptimizer(parsed: any, target: HeldTradeRow, suggestedPrice: number): any {
  return {
    ...
    listing: {
      ...
      peakSeasonPriceEur: Math.round(Number(parsed?.listing?.peak_season_price_eur ?? suggestedPrice * 1.2)),
      offSeasonPriceEur: Math.round(Number(parsed?.listing?.off_season_price_eur ?? suggestedPrice * 0.85)),
      seasonalityScore: Math.max(0, Math.min(100, Number(parsed?.listing?.seasonality_score ?? 50))),
      optimalSellWindow: String(parsed?.listing?.optimal_sell_window ?? '').slice(0, 7),
      seasonalityGrade: ['A', 'B', 'C', 'D', 'F'].includes(String(parsed?.listing?.seasonality_grade)) ? String(parsed.listing.seasonality_grade) : 'C',
    },
    seasonalityProfile: (parsed?.seasonalityProfile || []).slice(0, 12).map((s: any) => ({
      month: Math.max(1, Math.min(12, Number(s?.month ?? 1))),
      monthName: String(s?.month_name ?? '').slice(0, 3),
      demandLevel: (DEMAND_LEVELS as readonly string[]).includes(String(s?.demand_level)) ? String(s.demand_level) : 'medium',
      demandPct: Math.max(0, Math.min(100, Number(s?.demand_pct ?? 50))),
      avgPriceMultiplier: Math.max(0.5, Math.min(2.0, Number(s?.avg_price_multiplier ?? 1.0))),
      competitionLevel: ['high', 'medium', 'low'].includes(String(s?.competition_level)) ? String(s.competition_level) : 'medium',
      recommendedAction: ['sell_now', 'hold', 'list', 'delist'].includes(String(s?.recommended_action)) ? String(s.recommended_action) : 'hold',
    })),
    peakWindows: (parsed?.peakWindows || []).slice(0, 8).map((w: any) => ({
      seasonType: (SEASON_TYPES as readonly string[]).includes(String(w?.season_type)) ? String(w.season_type) : 'holiday',
      startMonth: Math.max(1, Math.min(12, Number(w?.start_month ?? 1))),
      endMonth: Math.max(1, Math.min(12, Number(w?.end_month ?? 1))),
      peakMonth: Math.max(1, Math.min(12, Number(w?.peak_month ?? 1))),
      expectedDemandLiftPct: Math.max(0, Math.min(100, Number(w?.expected_demand_lift_pct ?? 20))),
      expectedPriceLiftPct: Math.max(0, Math.min(50, Number(w?.expected_price_lift_pct ?? 10))),
      daysUntilPeak: Math.max(0, Number(w?.days_until_peak ?? 30)),
      preparationDays: Math.max(1, Number(w?.preparation_days ?? 7)),
    })),
    recommendations: (parsed?.recommendations || []).slice(0, 8).map((r: any) => ({
      action: String(r?.action ?? '').slice(0, 300),
      timing: ['immediate', 'within_7d', 'within_30d', 'within_90d'].includes(String(r?.timing)) ? String(r.timing) : 'within_30d',
      expectedRevenueLiftEur: Math.round(Number(r?.expected_revenue_lift_eur ?? 0)),
      confidencePct: Math.max(0, Math.min(100, Number(r?.confidence_pct ?? 60))),
    ...
    mlModels: (parsed?.mlModels || []).slice(0, 5).map((m: any) => ({
      ...
      predictionType: ['demand_forecast', 'price_forecast', 'seasonality_detection', 'trend_analysis'].includes(String(m?.prediction_type)) ? String(m.prediction_type) : 'demand_forecast',
      weightInEnsemble: Math.max(0, Math.min(100, Number(m?.weight_in_ensemble ?? 20))),
    })),
    summary: {
      seasonalityOptimizationScore: Math.max(0, Math.min(100, Number(parsed?.summary?.seasonality_optimization_score ?? 50))),
      seasonalityGrade: ['A', 'B', 'C', 'D', 'F'].includes(String(parsed?.summary?.seasonality_grade)) ? String(parsed.summary.seasonality_grade) : 'C',
      currentDemandLevel: (DEMAND_LEVELS as readonly string[]).includes(String(parsed?.summary?.current_demand_level)) ? String(parsed.summary.current_demand_level) : 'medium',
      optimalSellMonth: String(parsed?.summary?.optimal_sell_month ?? '').slice(0, 7),
      expectedRevenueLiftEur: Math.round(Number(parsed?.summary?.expected_revenue_lift_eur ?? 0)),
      biggestSeasonalityRisk: String(parsed?.summary?.biggest_seasonality_risk ?? '').slice(0, 200),
      biggestSeasonalityOpportunity: String(parsed?.summary?.biggest_seasonality_opportunity ?? '').slice(0, 200),
      quickestSeasonalityWin: String(parsed?.summary?.quickest_seasonality_win ?? '').slice(0, 200),
      seasonalityAnalysisScore: Math.max(0, Math.min(100, Number(parsed?.summary?.seasonality_analysis_score ?? 50))),
    },
  };
}
