// v7.47: Tax-Aware Selling Advisor — optimiziraj kdaj prodati za min davčni breme.
//
...
export async function GET() {
  try {
    ...
    const realized = soldThisYear.map(t => ({
    ...
    const heldPotential = heldTrades.map(t => {
    ...
    let sellPriority: Array<{ tradeId: string; title: string; reason: string; urgency: 'high' | 'medium' | 'low' }> = [];
    ...
    logger.error('/api/ai/tax-aware-selling', 'GET handler failed', err);
