// v6.59: AI Inventory Health Monitor v2 — real-time health z ML anomaly detection in predictive alerts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const includePredictions = Boolean(body?.includePredictions ?? true);
    ...
    if (heldTrades.length === 0 && soldTradesCurrent.length === 0) {
      return NextResponse.json({ ok: true, monitor: null, message: 'Ni podatkov za health monitoring.' });
    }
    ...
    // Category breakdown
    const catMap = new Map<string, { heldCount: number; soldCount: number; revenue: number; profit: number }>();
    ...
    const catStr = Array.from(catMap.entries()).slice(0, 8).map(([cat, c]) =>
      `- ${cat}: ${c.heldCount} held, ${c.soldCount} sold, ${Math.round(c.revenue)}€ revenue, ${Math.round(c.profit)}€ profit`
    ).join('\n');

    const prompt = `Si AI inventory health monitor v2 z ML anomaly detection in predictive alerts.
Real-time health monitoring z 8-metric scoring in predictive warnings.

TRENUTNO STANJE:
- Held items: ${totalItems} (skupna vrednost ${totalValue}€, investirano ${totalInvested}€)
- Povp dni v skladišču: ${avgDaysHeld}
- Stale (>30d): ${staleItems}, Critical (>90d): ${criticalItems}, Dead (>180d): ${deadItems}

PERFORMANCE (zadnji ${days} dni):
- Prodano: ${soldTradesCurrent.length} itemov
- Prihodek: ${Math.round(currentRevenue)}€
- Dobiček: ${Math.round(currentProfit)}€ (marža ${currentMarginPct}%)
- Prejšnje obdobje prihodek: ${Math.round(prevRevenue)}€
...
Odgovori LE z JSON:
{
  ...
  "summary": {
    ...
    const monitor = {
      ...
      categories: (parsed?.categories || []).slice(0, 8).map((c: any) => {
        const orig = Array.from(catMap.entries()).find(([cat]) => cat === String(c?.category));
        return {
      ...
      summary: {
        totalItemsMonitored: totalItems,
        totalInventoryValueEur: Math.round(Number(parsed?.summary?.total_inventory_value_eur ?? totalValue)),
        totalInvestedEur: Math.round(Number(parsed?.summary?.total_invested_eur ?? totalInvested)),
        totalUnrealizedProfitEur: Math.round(Number(parsed?.summary?.total_unrealized_profit_eur ?? totalValue - totalInvested)),
        healthyItemsCount: Math.max(0, Number(parsed?.summary?.healthy_items_count ?? totalItems - staleItems)),
        warningItemsCount: Math.max(0, Number(parsed?.summary?.warning_items_count ?? staleItems - criticalItems)),
        criticalItemsCount: Math.max(0, Number(parsed?.summary?.critical_items_count ?? criticalItems - deadItems)),
        deadItemsCount: Math.max(0, Number(parsed?.summary?.dead_items_count ?? deadItems)),
  ...
  } catch (e: any) { logger.error("/api/ai/inventory-health-monitor-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
