// v6.74: AI Buyer Lifetime Value Optimizer — optimizira LTV z ML in retention strategies
// POST /api/ai/buyer-lifetime-value-optimizer
// Body: { customerName?: string, monthsAhead?: number }
// Returns: { ok, optimizer: { buyers, ltvProjections, retentionStrategies, interventions, mlModels, summary } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const monthsAhead = Math.max(1, Math.min(36, Number(body?.monthsAhead ?? 24)));
    ...
    if (soldTrades.length === 0) return NextResponse.json({ ok: true, optimizer: null, message: 'Ni prodaj za LTV optimizacijo.' });
    ...
    const prompt = `Si AI buyer lifetime value optimizer z ML in retention strategies.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/buyer-lifetime-value-optimizer", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
