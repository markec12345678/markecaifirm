 ...
 */
// v6.54 / v8.96.1-batch4: AI Profit Margin Forecaster — napove maržo za naslednje N nakupov in kategorije
...
interface ProfitMarginForecasterInput {
  monthsAhead: number;
...
export const POST = withAiRoute<ProfitMarginForecasterInput>({
  endpoint: '/api/ai/profit-margin-forecaster',
  maxDuration: 90,
  enforceBudget: true,
  ...
  parseBody: async (req) => {
    ...
    return { monthsAhead, plannedPurchases };
  },

  handler: async (input, ctx: AiRouteContext) => {
    const { db, callAi, parseAi } = ctx;
    logDeprecatedCall('/api/ai/profit-margin-forecaster', ctx.req, '/api/ai/profit-margin-forecaster-pro');

    const { monthsAhead, plannedPurchases } = input;
    ...
    if (soldTrades12m.length === 0) {
      return apiOk({ ok: true, forecaster: null, message: 'Ni prodaj za margin forecast.' });
    }
    ...
    const categoryStats = computeCategoryStats(soldTrades12m);

    const prompt = buildPrompt({ monthsAhead, plannedPurchases, currentStats, recentStats, monthlyAvgProfit, categoryStats });
    const raw = await callAi(prompt);

    const parsed: any = parseAi(raw);
    const validCategories = new Set(categoryStats.map(c => c.category));
    const forecaster = transformForecaster(parsed, currentStats, monthlyAvgProfit, validCategories, categoryStats, monthsAhead);
...
function calcStats(trades: SoldTradeRow[]): Stats {
...
function computeCategoryStats(soldTrades12m: SoldTradeRow[]): CategoryStat[] {
...
function buildPrompt(d: PromptData): string {
  const catStr = d.categoryStats.slice(0, 8).map(c =>
    `- ${c.category} | ${c.count}x | profit ${c.profit}€ | revenue ${c.revenue}€ | margin ${c.marginPct}% | povp ${c.avgDaysToSell}d`
  ).join('\n');

  return `Si AI profit margin forecaster za slovenske oglasne platforme.
Napove maržo in profit za naslednjih ${d.monthsAhead} mesecev in ${d.plannedPurchases} načrtovanih nakupov.

TRENUTNO STANJE (zadnjih 12 mesecev):
- Skupno prodano: ${d.currentStats.count} itemov
- Skupni prihodek: ${d.currentStats.revenue}€
- Skupni cost: ${d.currentStats.cost}€
- Skupni profit: ${d.currentStats.profit}€
- Povprečna marža: ${d.currentStats.marginPct}%
- Povp dni do prodaje: ${d.currentStats.avgDaysToSell}
- Mesečni povprečni profit: ${Math.round(d.monthlyAvgProfit)}€

ZADNJI 90 DNI:
- ${d.recentStats.count} prodanih itemov
- Profit: ${d.recentStats.profit}€
- Marža: ${d.recentStats.marginPct}%
...
Odgovori LE z JSON:
{
  ...
  "summary": {
    "current_monthly_profit_eur": <number>,
    "projected_monthly_profit_${d.monthsAhead}m_eur": <number>,
