// v6.64: AI Inventory Seasonal Planner v2 — advanced seasonal planning z ML in cross-category analysis
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (soldTrades.length === 0) return NextResponse.json({ ok: true, planner: null, message: 'Ni podatkov za seasonal planning.' });
    ...
    const prompt = `Si AI inventory seasonal planner v2 z ML in cross-category seasonal analysis.
...
Odgovori LE z JSON:
{
  ...
  "summary": {
    ...
    const planner = {
      ...
      seasons: (parsed?.seasons || []).slice(0, 8).map((s: any) => ({
        season: SEASONS.includes(String(s?.season) as any) ? String(s.season) : 'spring',
      ...
      calendar: (parsed?.calendar || []).slice(0, 12).map((c: any) => ({
        ...
        season: SEASONS.includes(String(c?.season) as any) ? String(c.season) : 'spring',
      ...
      categories: (parsed?.categories || []).filter((c: any) => validCats.has(String(c?.category ?? ''))).slice(0, 12).map((c: any) => ({
        ...
        peakSeason: SEASONS.includes(String(c?.peak_season) as any) ? String(c.peak_season) : 'summer',
      ...
      mlPredictions: (parsed?.ml_predictions || []).slice(0, 5).map((m: any) => ({
        ...
        bestForSeason: SEASONS.includes(String(m?.best_for_season) as any) ? String(m.best_for_season) : 'summer',
      ...
      summary: {
        ...
        currentSeason: SEASONS.includes(String(parsed?.summary?.current_season) as any) ? String(parsed.summary.current_season) : 'summer',
        peakSeasonComing: SEASONS.includes(String(parsed?.summary?.peak_season_coming) as any) ? String(parsed.summary.peak_season_coming) : 'christmas',
  ...
  } catch (e: any) { logger.error("/api/ai/inventory-seasonal-planner-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
