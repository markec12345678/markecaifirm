// v6.42 / v8.95.5-other: AI Deal Velocity Accelerator — pospeši hitrost poslov od odkritja do prodaje.
...
export const POST = withAiRoute<Input>({
  endpoint: '/api/ai/deal-accelerator',
  maxDuration: 90,
  ...
  handler: async (_input, ctx: AiRouteContext) => {
    ...
    if (heldTrades.length === 0 && soldTrades.length === 0) {
      return apiOk({ ok: true, accelerator: null, message: 'Ni podatkov za deal accelerator.' });
    }

    const avgDays = computeAvgDays(soldTrades);
    const items = buildItems(heldTrades);
    const prompt = buildPrompt(items, heldTrades.length, soldTrades.length, avgDays);
...
function computeAvgDays(soldTrades: SoldTradeRow[]): number {
...
function buildPrompt(items: HeldItem[], heldCount: number, soldCount: number, avgDays: number): string {
  const itemsStr = items.slice(0, 15).map(i => `- [${i.id}] ${i.title} | ${i.category} | ${i.daysHeld}d | est ${i.estValue}€ | deal ${i.dealScore}`).join('\n');

  return `Si AI deal velocity accelerator. Pospeši hitrost poslov od odkritja do prodaje.

TRENUTNO: povp. ${avgDays}d do prodaje, ${heldCount} held, ${soldCount} sold
...
function transformAccelerator(parsed: any, items: HeldItem[], avgDays: number): any {
  ...
  return {
    ...
    accelerators: (parsed?.accelerators || []).slice(0, 8).map((a: any) => ({
      ...
      difficulty: includes(DIFFICULTIES, String(a?.difficulty)) ? String(a.difficulty) : 'medium',
    })),
    items: (parsed?.items || []).filter((it: any) => validIds.has(String(it?.id ?? ''))).map((it: any) => ({
      ...
      priority: includes(PRIORITIES, String(it?.priority)) ? String(it.priority) : 'medium',
    })),
    projectedSpeedup: {
