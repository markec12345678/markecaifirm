// v6.48: AI Bulk Listing Generator — generira optimized listings za več platform hkrati iz enega itema
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (heldTrades.length === 0) {
      return NextResponse.json({ ok: true, generator: null, message: 'Ni held tradeov za bulk listing generacijo.' });
    }
    ...
    const prompt = `Si AI bulk listing generator za slovenske in mednarodne oglasne platforme.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/bulk-listing-generator", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
