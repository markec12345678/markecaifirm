// v6.56 / v8.95.9-buyer-medium: AI Buyer Persona Generator v2 — napredne osebe z ML clustering in behavioral modeling
...
interface BuyerPersonaGeneratorV2Input {
  tradeId: string | null;
...
const PERSONA_TYPES = [
  'bargain_hunter', 'collector', 'parent_family', 'student_young',
  'professional', 'hobbyist', 'gift_giver', 'reseller',
  'tech_enthusiast', 'seasonal_buyer',
] as const;

const CHANNELS = ['bolha', 'facebook', 'vinted', 'email', 'sms', 'social', 'in_person'] as const;
const CLUSTER_MATCHES = ['high_value', 'repeat_loyal', 'one_time_buyer', 'new'] as const;
const GENDERS = ['male', 'female', 'any'] as const;
const PURCHASE_PATTERNS = ['impulse', 'deliberate', 'comparison', 'research'] as const;
const SENSITIVITY_LEVELS = ['low', 'medium', 'high'] as const;
const NEGOTIATION_TENDENCIES = ['aggressive', 'moderate', 'passive'] as const;
const PRIMARY_TRIGGERS = ['urgency', 'value', 'quality', 'emotion', 'status', 'convenience'] as const;
const TONES = ['professional', 'friendly', 'urgent', 'emotional', 'technical', 'playful'] as const;
const URGENCY_LEVELS = ['low', 'medium', 'high'] as const;

export const POST = withAiRoute<BuyerPersonaGeneratorV2Input>({
  endpoint: '/api/ai/buyer-persona-generator-v2',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    const { tradeId, category } = input;

    // 1. Pridobi held trade (target item)
    const targetItem = tradeId ? await loadTargetItem(db, tradeId) : null;
    ...
    if (soldTrades.length === 0 && !targetItem) {
      return apiOk({ ok: true, generator: null, message: 'Ni podatkov za persona generacijo.' });
    }

    // 3. Cluster analysis — find behavioral patterns
    const clusters = buildClusters(soldTrades);

    const prompt = buildPrompt(targetItem, clusters, category);
    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);

    const generator = transformGenerator(parsed);
...
async function loadTargetItem(db: AiRouteContext['db'], tradeId: string): Promise<TargetItemInfo | null> {
...
function buildClusters(soldTrades: SoldTradeRow[]): ClusterInfo[] {
...
function buildPrompt(targetItem: TargetItemInfo | null, clusters: ClusterInfo[], category: string | null): string {
  ...
  return `Si AI buyer persona generator v2 z ML clustering in behavioral modeling.
...
function transformGenerator(parsed: any): any {
  return {
    insights: String(parsed?.insights ?? '').slice(0, 500),
    personas: (parsed?.personas || []).slice(0, 7).map((p: any) => ({
      personaName: String(p?.persona_name ?? '').slice(0, 100),
      personaType: includes(PERSONA_TYPES, String(p?.persona_type)) ? String(p.persona_type) : 'bargain_hunter',
      demographics: {
        ageRange: String(p?.demographics?.age_range ?? '').slice(0, 30),
        gender: includes(GENDERS, String(p?.demographics?.gender)) ? String(p.demographics.gender) : 'any',
      ...
      behavioral: {
        purchasePattern: includes(PURCHASE_PATTERNS, String(p?.behavioral?.purchase_pattern)) ? String(p.behavioral.purchase_pattern) : 'deliberate',
        decisionTimeDays: Math.max(0, Number(p?.behavioral?.decision_time_days ?? 7)),
        priceSensitivity: includes(SENSITIVITY_LEVELS, String(p?.behavioral?.price_sensitivity)) ? String(p.behavioral.price_sensitivity) : 'medium',
        brandLoyalty: includes(SENSITIVITY_LEVELS, String(p?.behavioral?.brand_loyalty)) ? String(p.behavioral.brand_loyalty) : 'medium',
        researchDepth: includes(SENSITIVITY_LEVELS, String(p?.behavioral?.research_depth)) ? String(p.behavioral.research_depth) : 'medium',
        negotiationTendency: includes(NEGOTIATION_TENDENCIES, String(p?.behavioral?.negotiation_tendency)) ? String(p.behavioral.negotiation_tendency) : 'moderate',
      },
      motivational: {
        primaryTrigger: includes(PRIMARY_TRIGGERS, String(p?.motivational?.primary_trigger)) ? String(p.motivational.primary_trigger) : 'value',
      ...
      messaging: {
        tone: includes(TONES, String(p?.messaging?.tone)) ? String(p.messaging.tone) : 'professional',
      ...
      channels: {
        primary: includes(CHANNELS, String(p?.channels?.primary)) ? String(p.channels.primary) : 'email',
      ...
      clusterMatch: includes(CLUSTER_MATCHES, String(p?.cluster_match)) ? String(p.cluster_match) : 'new',
    ...
    messaging: (parsed?.messaging || []).slice(0, 8).map((m: any) => ({
      personaType: includes(PERSONA_TYPES, String(m?.persona_type)) ? String(m.persona_type) : 'bargain_hunter',
      ...
      urgencyLevel: includes(URGENCY_LEVELS, String(m?.urgency_level)) ? String(m.urgency_level) : 'medium',
    })),
    channels: (parsed?.channels || []).slice(0, 7).map((c: any) => ({
      channel: includes(CHANNELS, String(c?.channel)) ? String(c.channel) : 'email',
    ...
    summary: {
      ...
      bestChannelOverall: includes(CHANNELS, String(parsed?.summary?.best_channel_overall)) ? String(parsed.summary.best_channel_overall) : 'email',
