// v6.49: AI Buyer Matchmaker v2 — ML matching kupcev z inventarjem z behavioral scoringom
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (heldTrades.length === 0) {
      return NextResponse.json({ ok: true, matcher: null, message: 'Ni held tradeov za buyer matching.' });
    }
    ...
    if (soldTrades.length === 0) {
      return NextResponse.json({ ok: true, matcher: null, message: 'Ni prodaj za buyer profile.' });
    }
    ...
    if (customerName) {
      ...
      if (filtered.length === 0) {
        return NextResponse.json({ ok: true, matcher: null, message: `Kupec "${customerName}" ni najden v zgodovini.` });
      }
    }
    ...
    const prompt = `Si AI buyer matchmaker v2 z ML matching algoritmom.
...
Odgovori LE z JSON:
{
  ...
  "summary": {
    ...
    const matcher = {
      ...
      matches: (parsed?.matches || [])
        ...
        .map((m: any) => ({
          ...
          matchType: ['direct_match', 'cross_sell_match', 'upsell_match', 'repeat_match', 'new_category_match', 'reactivation_match'].includes(String(m?.match_type)) ? String(m.match_type) : 'direct_match',
          ...
          recommendedChannel: ['email', 'sms', 'in_person', 'social_dm', 'none'].includes(String(m?.recommended_channel)) ? String(m.recommended_channel) : 'email',
      ...
      scoringFactors: (parsed?.scoring_factors || []).slice(0, 8).map((s: any) => ({
        factor: ['category_fit', 'price_fit', 'recency', 'frequency', 'affinity', 'conversion', 'engagement', 'seasonal'].includes(String(s?.factor)) ? String(s.factor) : 'affinity',
      ...
      predictions: (parsed?.predictions || []).slice(0, 5).map((p: any) => ({
        metric: ['total_reach', 'total_responses', 'total_conversions', 'expected_revenue'].includes(String(p?.metric)) ? String(p.metric) : 'total_reach',
    ...
    return NextResponse.json({ ok: true, matcher });
  } catch (e: any) { logger.error("/api/ai/buyer-matchmaker-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
