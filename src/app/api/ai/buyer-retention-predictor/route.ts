// v6.51 / v8.95.9-competitor: AI Buyer Retention Predictor — napove churn in predlaga win-back strategije
...
interface BuyerRetentionInput {
  customerName: string | null;
  daysAhead: number;
}

export const POST = withAiRoute<BuyerRetentionInput>({
  endpoint: '/api/ai/buyer-retention-predictor',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, predictor: null, message: 'Ni prodaj za retention analizo.' });
    }

    // 2. Buyer aggregation + hevristika za churn in retention
    const buyers = buildBuyerRetentions(soldTrades);
...
function buildPrompt(targetBuyers: BuyerRetention[]): string {
  ...
  return `Si AI buyer retention predictor za slovenske oglasne platforme.
...
function transformPredictor(parsed: any, targetBuyers: BuyerRetention[]): any {
