// v6.67: AI Listing Image Quality Assessor v2 — VLM analiza z ML scoring in improvement roadmap
// POST /api/ai/listing-image-quality-assessor-v2
// Body: { tradeId?: string, listingId?: string }
// Returns: { ok, assessor: { listings, qualityFactors, improvements, shotPlan, editingPlan, summary } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    let targetListings: Array<{ id: string; title: string; category: string; imageUrl: string }> = [];
    if (tradeId) {
      const t = await db.trade.findUnique({ where: { id: tradeId }, select: { id: true, title: true, category: true, listing: { select: { imageUrl: true, detailImages: true } } } });
      ...
      const held = await db.trade.findMany({ where: { status: 'held' }, select: { id: true, title: true, category: true, listing: { select: { imageUrl: true } } }, take: 10, orderBy: { buyDate: 'desc' } });
      targetListings = held.map(t => ({ id: t.id, title: t.title, category: t.category || 'drugo', imageUrl: t.listing?.imageUrl ?? '' }));
    }
    if (targetListings.length === 0) return NextResponse.json({ ok: true, assessor: null, message: 'Ni listingov za image quality assessment.' });

    const settings = await getSettingsRow();
    const aiSettings: AiSettings = { provider: settings.aiProvider as AiProviderType, baseUrl: settings.aiBaseUrl, apiKey: settings.aiApiKey, model: settings.aiModel, fallbackProvider: (settings.fallbackProvider || '') as AiProviderType | '', fallbackBaseUrl: settings.fallbackBaseUrl || '', fallbackApiKey: settings.fallbackApiKey || '', fallbackModel: settings.fallbackModel || '' };

    const itemsStr = targetListings.slice(0, 10).map(i => `- [${i.id}] "${i.title}" | ${i.category} | slika: ${i.imageUrl ? 'da' : 'ne'}`).join('\n');

    const prompt = `Si AI listing image quality assessor v2 z VLM in ML scoring.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/listing-image-quality-assessor-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
