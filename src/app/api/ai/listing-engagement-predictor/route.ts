// v6.76: AI Listing Engagement Predictor — ML napoved engagement z 10 dimenzionalno analizo
// POST /api/ai/listing-engagement-predictor
// Body: { tradeId?: string }
// Returns: { ok, predictor: { listings, engagementFactors, predictions, optimizations, mlModels, summary } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const heldTrades = await db.trade.findMany({ where, select: { id: true, title: true, category: true, buyPrice: true, buyFees: true, buyDate: true, listing: { select: { aiEstimatedValue: true, price: true, imageUrl: true, description: true, detailDescription: true, dealScore: true, aiScore: true, aiRisk: true, location: true } } }, take: tradeId ? 1 : 20 });
    if (heldTrades.length === 0) return NextResponse.json({ ok: true, predictor: null, message: 'Ni held tradeov za engagement prediction.' });

    const settings = await getSettingsRow();
    const aiSettings: AiSettings = { provider: settings.aiProvider as AiProviderType, baseUrl: settings.aiBaseUrl, apiKey: settings.aiApiKey, model: settings.aiModel, fallbackProvider: (settings.fallbackProvider || '') as AiProviderType | '', fallbackBaseUrl: settings.fallbackBaseUrl || '', fallbackApiKey: settings.fallbackApiKey || '', fallbackModel: settings.fallbackModel || '' };

    const items = heldTrades.map(t => ({ id: t.id, title: t.title, category: t.category || 'drugo', price: t.listing?.price ?? Math.round(t.buyPrice * 1.25), estValue: t.listing?.aiEstimatedValue ?? Math.round((t.buyPrice + (t.buyFees ?? 0)) * 1.25), imageUrl: t.listing?.imageUrl ?? '', location: t.listing?.location ?? '', description: (t.listing?.detailDescription || t.listing?.description || '').slice(0, 200) }));
    const itemsStr = items.slice(0, 15).map(i => `- [${i.id}] "${i.title}" | ${i.category} | ${i.price}€ | ${i.location} | slika: ${i.imageUrl ? 'da' : 'ne'}`).join('\n');

    const prompt = `Si AI listing engagement predictor z ML in 10-dimenzionalno analizo.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/listing-engagement-predictor", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
