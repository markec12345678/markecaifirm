 ...
 */
// v6.51 / v8.96.2-batch3: AI Listing Description Generator v2 — multi-platform, multi-tone opisi z A/B variantami
...
interface ListingDescriptionGeneratorV2Input {
  tradeId: string | null;
  platforms: Platform[];
  tones: Tone[];
}

export const POST = withAiRoute<ListingDescriptionGeneratorV2Input>({
  endpoint: '/api/ai/listing-description-generator-v2',
  maxDuration: 120,
  ...
  parseBody: async (req) => {
    ...
    return { tradeId, platforms, tones };
  },
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    const { tradeId, platforms, tones } = input;

    // logDeprecatedCall PRESERVED — Phase 2 deprecation logging (ctx.req uporabljen namesto req parametra)
    logDeprecatedCall('/api/ai/listing-description-generator-v2', ctx.req, '/api/ai/listing-description-generator-v3');
    ...
    if (heldTrades.length === 0) {
      return apiOk({ ok: true, generator: null, message: 'Ni held tradeov za opis generacijo.' });
    }

    // 2. Pripravi item podatke
    const items = prepareItems(heldTrades);
    const prompt = buildPrompt(items, platforms, tones);

    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);
    const generator = transformGenerator(parsed, items, platforms, tones);
...
function prepareItems(
  heldTrades: Array<{
    ...
    listing: {
      ...
      aiEstimatedValue: number | null; location: string | null; aiImageAnalysis: string | null;
    } | null;
  }>,
): PreparedItem[] {
...
function buildPrompt(items: PreparedItem[], platforms: Platform[], tones: Tone[]): string {
  ...
  return `Si AI listing description generator v2 za slovenske in mednarodne oglasne platforme.
