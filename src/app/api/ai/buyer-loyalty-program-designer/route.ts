// v6.64 / v8.95.4-batch4: AI Buyer Loyalty Program Designer — oblikuje loyalty program z ML in tier-based rewards
...
interface BuyerLoyaltyProgramInput {
  monthsAhead: number;
}
...
export const POST = withAiRoute<BuyerLoyaltyProgramInput>({
  endpoint: '/api/ai/buyer-loyalty-program-designer',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, designer: null, message: 'Ni prodaj za loyalty program design.' });
    }
    ...
    const prompt = buildPrompt({ totalBuyers, repeatBuyers, highValueBuyers, avgBuyerValue });
    const raw = await callAi(prompt);
    const parsed: any = parseAi(raw);

    const designer = transformDesigner(parsed, { totalBuyers, monthsAhead });
...
function buildPrompt(ctx: StatsContext): string {
  const { totalBuyers, repeatBuyers, highValueBuyers, avgBuyerValue } = ctx;
  return `Si AI buyer loyalty program designer z ML in tier-based rewards.
...
function transformDesigner(parsed: any, ctx: { totalBuyers: number; monthsAhead: number }): any {
  const { totalBuyers, monthsAhead } = ctx;
  return {
