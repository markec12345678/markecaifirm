// v6.66 / v8.95.7-inv1: AI Inventory Capital Allocator — alokacija kapitala z ML in portfolio optimization
// Refaktoriran z withAiRoute helperjem (v8.94) + enforceBudget guard.
//
// POST /api/ai/inventory-capital-allocator
// Body: { totalBudget?: number, riskTolerance?: string }
// Returns: { ok, allocator: { current, allocation, scenarios, mlModels, rebalancing, summary } | null }
...
const CONCENTRATION_RISKS = ['low', 'medium', 'high'] as const;
const REBALANCE_DIRECTIONS = ['increase', 'decrease', 'maintain'] as const;
const SCENARIO_TYPES = ['conservative', 'balanced', 'aggressive'] as const;
const ML_MODELS = ['mean_variance', 'kelly_criterion', 'risk_parity', 'momentum_tilting', 'ensemble'] as const;
const REBALANCE_ACTIONS = ['buy_more', 'sell_partial', 'exit_category', 'enter_new', 'hold'] as const;
...
interface AllocatorTotals {
  totalInvested: number;
  totalValue: number;
}

export const POST = withAiRoute<CapitalAllocatorInput>({
  endpoint: '/api/ai/inventory-capital-allocator',
  maxDuration: 90,
  enforceBudget: true, // AI klic — preveri budget + avtomatsko recordAiCall po uspehu

  parseBody: async (req) => {
    const body = await req.json().catch(() => ({}));
    const rtRaw = String(body?.riskTolerance ?? 'moderate');
    return {
      totalBudget: Math.max(0, Number(body?.totalBudget ?? 0)),
      riskTolerance: (RISK_TOLERANCES as readonly string[]).includes(rtRaw) ? rtRaw : 'moderate',
    };
  },

  // No validateInput — totalBudget clamp >= 0, riskTolerance fallback 'moderate'

  handler: async (input, ctx: AiRouteContext) => {
    const { db, callAi, parseAi } = ctx;
    const { totalBudget, riskTolerance } = input;
    ...
    if (soldTrades.length === 0) {
      return apiOk({ ok: true, allocator: null, message: 'Ni podatkov za capital allocation.' });
    }

    const categoryStats = computeCategoryStats(soldTrades);
    const totals = computeAllocatorTotals(heldTrades);

    const prompt = buildCapitalPrompt({
    ...
    const allocator = transformAllocator(parsed, categoryStats, totals);

    return apiOk({ ok: true, allocator });
  },
});

// --- Pomožne funkcije (čiste, testabilne) --------------------------------

function computeCategoryStats(soldTrades: SoldTradeRow[]): CategoryStat[] {
  ...
  for (const t of soldTrades) {
    ...
    c.count += 1; c.invested += cost; c.revenue += revenue; c.profit += revenue - cost; c.avgDaysToSell += days;
  }
  catMap.forEach(c => {
  ...
  return Array.from(catMap.entries())
    .map(([cat, c]) => ({ category: cat, ...c, invested: Math.round(c.invested), revenue: Math.round(c.revenue), profit: Math.round(c.profit) }))
    .sort((a, b) => b.profit - a.profit);
}

function computeAllocatorTotals(heldTrades: HeldTradeRow[]): AllocatorTotals {
...
function buildCapitalPrompt(input: CapitalPromptInput): string {
  const { totalInvested, totalValue, riskTolerance, totalBudget, categoryStats } = input;
  ...
  return `Si AI inventory capital allocator z ML in portfolio optimization theory.
...
function transformAllocator(parsed: any, categoryStats: CategoryStat[], totals: AllocatorTotals): any {
  const validCats = new Set(categoryStats.map(c => c.category));
  const avgMarginFallback = categoryStats.length > 0
    ? categoryStats.reduce((s, c) => s + c.marginPct, 0) / Math.max(1, categoryStats.length)
    : 0;
  return {
    insights: String(parsed?.insights ?? '').slice(0, 500),
    current: {
      totalCapitalEur: Math.round(Number(parsed?.current?.total_capital_eur ?? totals.totalInvested)),
      totalValueEur: Math.round(Number(parsed?.current?.total_value_eur ?? totals.totalValue)),
      totalCategories: Math.max(0, Number(parsed?.current?.total_categories ?? categoryStats.length)),
      avgMarginPct: Math.round(Number(parsed?.current?.avg_margin_pct ?? avgMarginFallback) * 10) / 10,
      ...
      concentrationRisk: (CONCENTRATION_RISKS as readonly string[]).includes(String(parsed?.current?.concentration_risk)) ? String(parsed.current.concentration_risk) : 'medium',
    },
    allocation: (parsed?.allocation || []).filter((a: any) => validCats.has(String(a?.category ?? ''))).slice(0, 10).map((a: any) => ({
      ...
      rebalanceDirection: (REBALANCE_DIRECTIONS as readonly string[]).includes(String(a?.rebalance_direction)) ? String(a.rebalance_direction) : 'maintain',
    ...
    scenarios: (parsed?.scenarios || []).slice(0, 3).map((s: any) => ({
      scenario: (SCENARIO_TYPES as readonly string[]).includes(String(s?.scenario)) ? String(s.scenario) : 'balanced',
    ...
    rebalancing: (parsed?.rebalancing || []).slice(0, 8).map((r: any) => ({
      action: (REBALANCE_ACTIONS as readonly string[]).includes(String(r?.action)) ? String(r.action) : 'hold',
    ...
    summary: {
      ...
      bestModel: (ML_MODELS as readonly string[]).includes(String(parsed?.summary?.best_model)) ? String(parsed.summary.best_model) : 'ensemble',
