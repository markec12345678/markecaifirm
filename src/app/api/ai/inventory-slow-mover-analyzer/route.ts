// v6.82 / v8.95.8-refactor: AI Inventory Slow Mover Analyzer — ML analiza počasi premikajočega inventarja
...
interface SlowMoverAnalyzerInput {
  days: number;
}
...
export const POST = withAiRoute<SlowMoverAnalyzerInput>({
  endpoint: '/api/ai/inventory-slow-mover-analyzer',
  maxDuration: 90,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    if (heldTrades.length === 0) {
      return apiOk({ ok: true, analyzer: null, message: 'Ni inventarja za slow mover analizo.' });
    }

    const soldTrades = await db.trade.findMany({
      where: { status: 'sold', sellDate: { gte: since, not: null } },
    ...
    const now = Date.now();
    const categoryAvgSellDays = computeCategoryAvgSellDays(soldTrades);
    const heldItems = buildHeldItems(heldTrades, categoryAvgSellDays, now);
    ...
    const analyzer = transformAnalyzer(parsed, { heldItems, totalValue, slowMovers, slowMoverValue, slowMoverPct });
...
function computeCategoryAvgSellDays(soldTrades: SoldTradeRow[]): Map<string, number> {
  ...
  for (const [cat, s] of categoryItems) {
    categoryAvgSellDays.set(cat, s.sold > 0 ? Math.round(s.totalDays / s.sold) : 0);
  }
  return categoryAvgSellDays;
}

function buildHeldItems(
  heldTrades: HeldTradeRow[],
  categoryAvgSellDays: Map<string, number>,
  now: number
): HeldItem[] {
...
interface SlowMoverPromptInput {
...
function buildPrompt(input: SlowMoverPromptInput): string {
  const { heldItemsCount, totalValue, slowMoversCount, slowMoverValue, slowMoverPct, categoryAvgSellDays, topSlow } = input;
  return `Si AI inventory slow mover analyzer z ML in predictive analytics.
Analizira počasi premikajoč se inventar z 5 tierji in 8 akcijskimi tipi.

STATS:
- Total items: ${heldItemsCount} | vrednost: ${Math.round(totalValue)}€
- Slow movers: ${slowMoversCount} | vrednost: ${Math.round(slowMoverValue)}€ (${slowMoverPct}%)
...
function transformAnalyzer(parsed: any, stats: SlowMoverStats) {
  const { heldItems, totalValue, slowMovers, slowMoverValue, slowMoverPct } = stats;
  return {
    insights: String(parsed?.insights ?? '').slice(0, 500),
    overview: {
      totalItems: Math.max(0, Number(parsed?.overview?.total_items ?? heldItems.length)),
      totalValueEur: Math.round(Number(parsed?.overview?.total_value_eur ?? totalValue)),
      slowMoversCount: Math.max(0, Number(parsed?.overview?.slow_movers_count ?? slowMovers.length)),
      slowMoversValueEur: Math.round(Number(parsed?.overview?.slow_movers_value_eur ?? slowMoverValue)),
      slowMoversPct: Math.max(0, Math.min(100, Number(parsed?.overview?.slow_movers_pct ?? slowMoverPct))),
      avgAgeDays: Math.max(0, Number(parsed?.overview?.avg_age_days ?? Math.round(heldItems.reduce((s, i) => s + i.ageDays, 0) / Math.max(1, heldItems.length)))),
      tiedUpCapitalEur: Math.round(Number(parsed?.overview?.tied_up_capital_eur ?? slowMoverValue)),
      slowMoverGrade: ['A', 'B', 'C', 'D', 'F'].includes(String(parsed?.overview?.slow_mover_grade)) ? String(parsed.overview.slow_mover_grade) : 'C',
    },
    slowMovers: (parsed?.slowMovers || []).slice(0, 15).map((s: any) => ({
      itemTitle: String(s?.item_title ?? '').slice(0, 200),
      category: String(s?.category ?? '').slice(0, 50),
      currentAgeDays: Math.max(0, Number(s?.current_age_days ?? 0)),
      categoryAvgDays: Math.max(0, Number(s?.category_avg_days ?? 0)),
      slownessRatio: Math.round(Number(s?.slowness_ratio ?? 0) * 100) / 100,
      slownessTier: (SLOWNESS_TIERS as readonly string[]).includes(String(s?.slowness_tier)) ? String(s.slowness_tier) : 'normal_mover',
      currentValueEur: Math.round(Number(s?.current_value_eur ?? 0)),
      predictedSellValueEur: Math.round(Number(s?.predicted_sell_value_eur ?? 0)),
      predictedDaysToSell: Math.max(0, Number(s?.predicted_days_to_sell ?? 0)),
      recommendedAction: (ACTION_TYPES as readonly string[]).includes(String(s?.recommended_action)) ? String(s.recommended_action) : 'discount_15',
    })),
    categoryAnalysis: (parsed?.categoryAnalysis || []).slice(0, 10).map((c: any) => ({
      ...
      slowMoversCount: Math.max(0, Number(c?.slow_movers_count ?? 0)),
      slowMoverPct: Math.max(0, Math.min(100, Number(c?.slow_mover_pct ?? 0))),
      avgAgeDays: Math.max(0, Number(c?.avg_age_days ?? 0)),
      tiedUpCapitalEur: Math.round(Number(c?.tied_up_capital_eur ?? 0)),
      primaryIssue: String(c?.primary_issue ?? '').slice(0, 200),
      categoryAction: (ACTION_TYPES as readonly string[]).includes(String(c?.category_action)) ? String(c.category_action) : 'discount_15',
    })),
    recommendations: (parsed?.recommendations || []).slice(0, 8).map((r: any) => ({
      action: String(r?.action ?? '').slice(0, 300),
      actionType: (ACTION_TYPES as readonly string[]).includes(String(r?.action_type)) ? String(r.action_type) : 'discount_15',
      targetItemsCount: Math.max(0, Number(r?.target_items_count ?? 0)),
      expectedRecoveryEur: Math.round(Number(r?.expected_recovery_eur ?? 0)),
      lossAcceptanceEur: Math.round(Number(r?.loss_acceptance_eur ?? 0)),
      implementationDays: Math.max(1, Number(r?.implementation_days ?? 7)),
      priority: ['high', 'medium', 'low'].includes(String(r?.priority)) ? String(r.priority) : 'medium',
    })),
    mlModels: (parsed?.mlModels || []).slice(0, 5).map((m: any) => ({
      model: ['random_forest', 'xgboost', 'lstm', 'prophet', 'ensemble'].includes(String(m?.model)) ? String(m.model) : 'ensemble',
      accuracyPct: Math.max(0, Math.min(100, Number(m?.accuracy_pct ?? 75))),
      predictionType: ['sell_time_prediction', 'value_degradation', 'risk_score', 'action_recommendation'].includes(String(m?.prediction_type)) ? String(m.prediction_type) : 'sell_time_prediction',
      weightInEnsemble: Math.max(0, Math.min(100, Number(m?.weight_in_ensemble ?? 20))),
    })),
    summary: {
      slowMoverRiskScore: Math.max(0, Math.min(100, Number(parsed?.summary?.slow_mover_risk_score ?? 50))),
      slowMoverGrade: ['A', 'B', 'C', 'D', 'F'].includes(String(parsed?.summary?.slow_mover_grade)) ? String(parsed.summary.slow_mover_grade) : 'C',
      totalTiedUpCapitalEur: Math.round(Number(parsed?.summary?.total_tied_up_capital_eur ?? slowMoverValue)),
      deadStockCount: Math.max(0, Number(parsed?.summary?.dead_stock_count ?? 0)),
      recoverableValueEur: Math.round(Number(parsed?.summary?.recoverable_value_eur ?? slowMoverValue * 0.6)),
      biggestSlowMoverRisk: String(parsed?.summary?.biggest_slow_mover_risk ?? '').slice(0, 200),
      biggestRecoveryOpportunity: String(parsed?.summary?.biggest_recovery_opportunity ?? '').slice(0, 200),
      quickestRecoveryWin: String(parsed?.summary?.quickest_recovery_win ?? '').slice(0, 200),
      slowMoverAnalysisScore: Math.max(0, Math.min(100, Number(parsed?.summary?.slow_mover_analysis_score ?? 50))),
    },
  };
}
