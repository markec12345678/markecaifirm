// v6.66 / v8.95.7-listing: AI Listing Cross-Platform Optimizer — optimizira oglase čez 5 platform z ML in sync strategy
// Refaktoriran z withAiRoute helperjem (v8.95.7-listing) + enforceBudget guard.
//
// POST /api/ai/listing-cross-platform-optimizer
// Body: { tradeId?: string }
// Returns: { ok, optimizer: { listings, platforms, syncStrategy, conflicts, performance, summary } | null }
...
interface ListingCrossPlatformOptimizerInput {
  tradeId: string | null;
}

export const POST = withAiRoute<ListingCrossPlatformOptimizerInput>({
  endpoint: '/api/ai/listing-cross-platform-optimizer',
  maxDuration: 120,
  enforceBudget: true,
  ...
  handler: async (input, ctx: AiRouteContext) => {
    ...
    const heldTrades = await db.trade.findMany({
      ...
      select: {
        ...
        listing: { select: { aiEstimatedValue: true, dealScore: true, location: true, description: true, detailDescription: true, imageUrl: true } },
      },
      take: tradeId ? 1 : 15,
    });

    if (heldTrades.length === 0) {
      return apiOk({ ok: true, optimizer: null, message: 'Ni held tradeov za cross-platform optimizacijo.' });
    }
...
function buildItems(heldTrades: HeldTradeRow[]): CrossPlatformItem[] {
...
function buildItemsStr(items: CrossPlatformItem[]): string {
  return items
    .slice(0, 10)
    .map(i => `- [${i.id}] "${i.title}" | ${i.category} | ${i.cost}€→${i.estValue}€ | ${i.location}`)
    .join('\n');
}

function buildPrompt(itemsCount: number, itemsStr: string): string {
  return `Si AI listing cross-platform optimizer z ML in sync strategy.
...
function transformOptimizer(parsed: any, items: CrossPlatformItem[]): any {
