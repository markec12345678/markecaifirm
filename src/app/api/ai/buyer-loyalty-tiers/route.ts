// v6.80: AI Buyer Loyalty Tiers — ML klasifikacija kupcev v loyalty tierje z rewards
// POST /api/ai/buyer-loyalty-tiers
// Body: { customerName?: string }
// Returns: { ok, analyzer: { buyers, tierDistribution, rewardsProgram, migrationPaths, mlModels, summary } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (soldTrades.length === 0) return NextResponse.json({ ok: true, analyzer: null, message: 'Ni prodaj za loyalty analizo.' });

    const buyerMap = new Map<string, { name: string; purchases: number; totalSpent: number; avgOrder: number; firstPurchase: Date | null; lastPurchase: Date | null; categories: Set<string>; lifetime_days: number }>();
    const now = Date.now();
    ...
    const buyers = Array.from(buyerMap.values()).map(b => { b.avgOrder = b.purchases > 0 ? Math.round(b.totalSpent / b.purchases) : 0; b.lifetime_days = b.firstPurchase ? Math.round((now - b.firstPurchase.getTime()) / (24*60*60*1000)) : 0; return b; });
    ...
    const prompt = `Si AI buyer loyalty tiers analyzer z ML in reward program design.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/buyer-loyalty-tiers", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
