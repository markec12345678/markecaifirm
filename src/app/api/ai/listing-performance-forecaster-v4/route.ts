// v6.59 / v8.96.3-batch2: AI Listing Performance Forecaster v4 — deep learning z transformer architecture
...
interface ListingPerformanceForecasterInput {
  tradeId: string | null;
  horizonDays: number;
}

export const POST = withAiRoute<ListingPerformanceForecasterInput>({
  endpoint: '/api/ai/listing-performance-forecaster-v4',
  maxDuration: 120,
  enforceBudget: true,
...
function buildPrompt(items: ForecasterItem[], itemsStr: string): string {
  return `Si AI listing performance forecaster v4 z deep learning (transformer architecture).
...
function transformForecaster(parsed: any, items: ForecasterItem[]): any {
