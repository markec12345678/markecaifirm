// v6.89: AI Buyer Engagement Predictor v2 — ML napoved engagementa kupcev z multi-channel scoring
// POST /api/ai/buyer-engagement-predictor-v2
// Body: { customerName?: string }
// Returns: { ok, predictor: { overview, buyers, engagementChannels, engagementDrivers, recommendations, mlModels, summary } }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (soldTrades.length === 0) return NextResponse.json({ ok: true, predictor: null, message: 'Ni prodaj za engagement analizo.' });
    ...
    const prompt = `Si AI buyer engagement predictor v2 z ML in multi-channel scoring.
...
Odgovori LE z JSON:
{
  ...
  } catch (e: any) { logger.error("/api/ai/buyer-engagement-predictor-v2", "POST handler failed", e); return NextResponse.json({ error: e?.message ?? 'Napaka' }, { status: 500 }); }
}
