/**
 * SeedAndTelegramCard — v8.35 lime + cyan gradient "onboarding" card.
 *
 * Extracted from the original `system-cards.tsx` (1947 lines) as part of
 * v8.94.7-split. Two action areas in one card:
 *
 *   A. SEED DEMO DATA — if Trade table is empty (0 trades), shows a prominent
 *      🌱 button to load 25 realistic Slovenian trades. Idempotent — if trades
 *      already exist, the section is hidden.
 *
 *   B. TELEGRAM BRAIN NOTIFICATIONS — 3 test buttons that send test
 *      notifications via the existing Telegram bot (digest/autopilot/anomaly).
 *      Each button returns "✅ Poslano" or "❌ Telegram ni konfiguriran"
 *      when Telegram is not set up in Settings.
 *
 * Placed IMMEDIATELY BELOW 🏥 System Health. Dual-tint gradient: lime for
 * seed (growth metaphor) + cyan for Telegram (messaging metaphor).
 * Auto-refreshes trade count every 60s.
 */
...
import { AlertOctagon, Bot, MessageCircle, RefreshCw, Send, Sprout } from 'lucide-react';
