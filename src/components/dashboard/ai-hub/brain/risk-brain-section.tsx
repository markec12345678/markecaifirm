/**
 * Risk Brain Section (v8.19, red/rose) — 6 risk signals.
 *
 * Extracted from the original `brain-sections.tsx` (1147 lines) as part of
 * v8.94.8-split-brain. Fetches `/api/ai/brain/risk`, displays
 * `oneLineSummary` + risk grade pill + biggest risk pill (with riskLevel
 * LOW/MEDIUM/HIGH/CRITICAL) + top 3 mitigation actions + 30d/90d STRUCTURED
 * projections (projectedRiskScore + projectedConcentrationPct +
 * projectedAgedPct + recommendedRiskBudget), and provides a refresh button.
 *
 * Projection shape: structured objects with risk score + budget.
 * Signal `score` is INVERTED (HIGHER = LOWER risk) and `expectedUpliftEUR`
 * is rendered as risk-reduction EUR (prefix `-`).
 */
...
import type { RiskBrainResult } from '../types';
