/**
 * Sourcing Brain Section (v8.18, violet) — 6 sourcing signals.
 *
 * Extracted from the original `brain-sections.tsx` (1147 lines) as part of
 * v8.94.8-split-brain. Fetches `/api/ai/brain/sourcing`, displays
 * `oneLineSummary` + sourcing grade pill + top 3 actions + per-source state
 * + 30d/90d STRUCTURED projections (recommendedSourceToScale +
 * recommendedSourceToReduce + projectedTotalMonthlyProfit +
 * projectedConcentrationPct + recommendedNewSource), and provides a refresh
 * button.
 *
 * Projection shape: structured objects with per-source recommendations
 * (distinct from all other Brains).
 */
