/**
 * BrainSnapshotsSection — historical validation phase (v8.23, emerald).
 *
 * Extracted from the original `snapshots-accuracy.tsx` (577 lines) as part of
 * v8.94.9-split. Horizontal-scroll list of past Master Brain snapshots
 * (date + grade + projection + actual comparison).
 *
 *   - GET  /api/ai/brain/snapshots?days=30  → list of SnapshotView rows
 *   - POST /api/ai/brain/snapshots { force: true } → manual save trigger
 *
 * Module-local types (SnapshotView, SnapshotsApiResponse, AccuracyTrendPoint,
 * AccuracyApiResponse, DOMAIN_TREND_LABELS) come from ./snapshot-types.
 * Color helpers come from ../../utils (gradeColor, riskLevelColor).
 */
...
import { AlertCircle, Camera, History, RefreshCw, Save } from 'lucide-react';
...
import type { SnapshotsApiResponse } from './snapshot-types';
