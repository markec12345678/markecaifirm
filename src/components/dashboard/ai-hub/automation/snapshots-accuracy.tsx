/**
 * BrainSnapshotsSection + AccuracyTrendCard — historical validation phase.
 *
 * Extracted from the original `automation-cards.tsx` (4095 lines) as part of
 * v8.94.6-split.
 *
 *   - BrainSnapshotsSection (v8.23, emerald) — horizontal-scroll list of past
 *     Master Brain snapshots (date + grade + projection + actual comparison).
 *     GET /api/ai/brain/snapshots, POST /api/ai/brain/snapshots { force: true }.
 *   - AccuracyTrendCard (v8.25, teal) — META-ANALYSIS: 30d/90d accuracy big
 *     numbers + Overall Health sparkline + 7-domain grade trend table.
 *     GET /api/ai/brain/accuracy?days=30, POST /api/ai/brain/accuracy/backfill.
 *
 * Module-local types (SnapshotsApiResponse, AccuracyApiResponse,
 * DOMAIN_TREND_LABELS) come from ./types. Color helpers come from ../utils.
 */
...
import { AlertCircle, Camera, History, RefreshCw, Save, TrendingUp } from 'lucide-react';
