#!/usr/bin/env python3
"""Update AI_ENDPOINTS.md with 3 new v6.79 endpoints, renumber all rows sequentially."""
import re
...
NEW_ENDPOINTS = [
    ("buyer-payment-reliability", "/api/ai/buyer-payment-reliability"),
    ("inventory-shrinkage-detector", "/api/ai/inventory-shrinkage-detector"),
    ("listing-question-optimizer", "/api/ai/listing-question-optimizer"),
]
...
def main():
    ...
    # Find header row pattern: | N | name | path |
    ...
    for line in lines:
        ...
        # Check if any new endpoint should be inserted before this row (alphabetical order)
        for new_name, new_path in NEW_ENDPOINTS:
            if not inserted[new_name] and new_name < current_name:
                # Insert new row (with placeholder number, will renumber after)
                new_lines.append(f"__NEW__{new_name}__{new_path}")
                inserted[new_name] = True
        new_lines.append(line)

    # Append any not yet inserted (at end)
    for new_name, new_path in NEW_ENDPOINTS:
    ...
    for line in new_lines:
        if line.startswith("__NEW__"):
            _, name, path = line.split("__", 2)[1], None, None
            # Better parse: __NEW__{name}__{path}
            parts = line.split("__")
            # parts = ['', 'NEW', name, path]
            name = parts[2]
            path = parts[3]
