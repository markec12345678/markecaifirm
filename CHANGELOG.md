...
### Added — 🚀 Aggressive Auto-pilot + Anomaly Detection (🎯 AUTOMATION PHASE COMPLETE)

**🎯 AUTOMATION PHASE COMPLETE.** v8.31 zaključi Automation phase z: (1) **Aggressive mode** (opt-in, double confirmation) — dovoli tudi MEDIUM confidence (ne le LOW), višji limiti (10/dan, 2000€/dan), uplift < 300€. HIGH confidence je še vedno vedno manual (oba modal). Domain='risk' je še vedno vedno izključen (oba modal). (2) **Anomaly detection** — če auto-pilot izvede >8 akcij v 1 uri, se samodejno suspendira (possible loop), zahteva manual re-enable. (3) **Double opt-in za aggressive mode** (5-min confirmation window). (4) **UI izboljšave** — mode selector, anomaly banner, threshold comparison box.

#### Aggressive Auto-pilot (mode-aware 8 pravila)

- **AGGRESSIVE_CONFIG** constant: `maxDailyLimit: 10`, `maxDailyBudgetEUR: 2000`, `maxUpliftEUR: 300`, `allowedConfidence: ['LOW', 'MEDIUM']`, `anomalyHourlyThreshold: 8`. HIGHER than safe (limit 5, budget 500€, uplift 100€, confidence LOW only).
- **SAFE_CONFIG** constant: mirror of v8.30 thresholds (limit 5, budget 500€, uplift 100€, confidence LOW only).
- **AGGRESSIVE_CONFIRM_WINDOW_MS = 5 * 60 * 1000** (5 minutes) — window for double confirmation.
- **checkAutoPilotEligibilityV2()** — mode-aware 8 pravila:
  1. Auto-pilot enabled (master switch) — both modes
  2. Mode is 'safe' or 'aggressive' (V2 accepts both — V1 only accepted 'safe')
  3. User risk tolerance != 'conservative' — both modes
  4. Confidence in mode's allowedConfidence (safe: ['LOW']; aggressive: ['LOW', 'MEDIUM']; HIGH vedno excluded — both modes)
  5. expectedUpliftEUR < mode threshold (safe: 100€; aggressive: 300€)
  6. Domain != 'risk' — both modes (risk mitigation = human judgment)
  7. Daily limit ≤ mode's maxDailyLimit (safe: 5; aggressive: 10)
  8. Daily budget ≤ mode's maxDailyBudgetEUR (safe: 500€; aggressive: 2000€)
  Returns 8 PASS/FAIL razlogov z mode-specific thresholds navedenimi v vsakem razlogu (npr. "FAIL: confidence MEDIUM not in [LOW] (safe mode)" ali "PASS: confidence MEDIUM allowed in aggressive mode").
- **runSafeAutoPilot() modified**: (1) po config load-a (če enabled), kliče `checkAnomaly()` — če suspended, return-a early z `{ok:true, anomalySuspended:true, anomalyReason, ...}` (NE proži auto-executions); (2) uporablja `checkAutoPilotEligibilityV2()` namesto V1; (3) po vsaki auto-execuciji kliče `incrementHourlyCounter()` — če `suspendedNow=true`, preostali pending drafts so označeni kot skipped z "FAIL: auto-pilot suspended mid-run (anomaly: ...)".

#### Anomaly Detection (hourly counter + suspension)

- **checkAnomaly()**: load-a `autoPilotHourlyExecCount` + `autoPilotHourlyWindowStart` iz Settings. Če window >1h, reset-a counter na 0 + windowStart=null (rolling 1-hour window). Če count ≥ `AGGRESSIVE_CONFIG.anomalyHourlyThreshold` (8), suspendira — set-a `autoPilotAnomalySuspended=true` + `autoPilotAnomalySuspendedAt=now` + `autoPilotAnomalyReason="N akcij v 1 uri — possible loop"`. Vrača `{anomaly: boolean, reason: string | null}`.
- **incrementHourlyCounter()**: load-a window + count. Če window null ali expired (>1h), reset-a count=1, windowStart=now. Else count++. Persist-a v Settings. Če count ≥ threshold, suspendira (set-a `autoPilotAnomalySuspended=true` + reason). Vrača `{count, suspended, suspendedNow, reason}` — `suspendedNow=true` pomeni da je bila suspension pravkar sprožena (mid-run).
- **clearAnomalySuspension()**: user manually re-enables. Set-a `autoPilotAnomalySuspended=false` + clear-a `autoPilotAnomalySuspendedAt` + `autoPilotAnomalyReason` + reset-a `autoPilotHourlyExecCount=0` + `autoPilotHourlyWindowStart=null`. NE spremeni `autoPilotEnabled` ali `autoPilotMode` — user lahko neodvisno toggle-a te preko config akcije.

#### Double Confirmation za Aggressive Mode (5-min window)

- **enableAggressiveMode()** flow:
  - **First call** (no pending confirmation): set-a `autoPilotAggressiveConfirmedAt=now` in vrača `{ok:true, confirmed:false, message:"⚠️ Aggressive mode dovoli MEDIUM confidence. Potrdi ponovno v 5 minutah.", confirmedAt:ISO}`.
  - **Second call within 5 min**: set-a `autoPilotMode='aggressive'`, clear-a `autoPilotAggressiveConfirmedAt`, clear-a anomaly suspension (to je "fresh start") in vrača `{ok:true, confirmed:true, message:"✅ Aggressive mode omogočen — dovoljena MEDIUM confidence (do 300€ uplift, 10/dan, 2000€/dan budget). HIGH je še vedno vedno manual."}`.
  - **After 5 min**: confirmation expires — treated as first call (set-a new pending confirmation).
  - **Guard**: requires `autoPilotEnabled=true` (master switch ON). If disabled, returns error message "Najprej vklopi auto-pilot (master switch OFF). Aggressive mode zahteva aktiven auto-pilot."
- **disableAggressiveMode()**: immediately set-a `autoPilotMode='safe'` + clear-a `autoPilotAggressiveConfirmedAt`. Single call, no confirmation needed — fail-safe (easier to disable than enable).

#### 6 new Settings fields (Prisma schema)

- `autoPilotAggressiveConfirmedAt` (DateTime?) — when user double-confirmed aggressive mode (null if none pending).
...
- `autoPilotHourlyExecCount` (Int @default(0)) — count of auto-executions in current 1-hour window.
- `autoPilotHourlyWindowStart` (DateTime?) — start of current 1-hour window (null if no executions yet).
- `bun run db:push` aplikirana na SQLite + Prisma Client regeneriran.
- SCHEMA_VERSION v `src/lib/db.ts` bump-an na `v8.31-aggressive-anomaly` (discard-a stale PrismaClient v globalThis cache).

#### 3 new API actions (existing 'run' in 'config' ostanejo)

- **POST `/api/ai/brain/auto-pilot` { action: 'enable_aggressive' }** → `enableAggressiveMode()`. Vrača `{ok, confirmed, message, confirmedAt?}`.
- **POST `/api/ai/brain/auto-pilot` { action: 'disable_aggressive' }** → `disableAggressiveMode()`. Vrača `{ok, mode:'safe'}`.
- **POST `/api/ai/brain/auto-pilot` { action: 'clear_anomaly' }** → `clearAnomalySuspension()`. Vrača `{ok, message}`.
- **UPDATE**: `'config'` akcija IGNORIRA `mode` field — mode changes morajo iti preko enable/disable_aggressive akcij (double-confirmation safety). Direct mode setting preko 'config' bi bypass-a safety mechanism.
- **GET `/api/ai/brain/auto-pilot`** sedaj vrača tudi 6 novih fields v `config`: `aggressiveConfirmedAt`, `anomalySuspended`, `anomalySuspendedAt`, `anomalyReason`, `hourlyExecCount`, `hourlyWindowStart` (potrebni za UI mode selector + anomaly banner).
- **`runSafeAutoPilot()`** vrača nov `anomalySuspended?: boolean` in `anomalyReason?: string | null` v result (populirano če je anomaly triggered pre-run ali mid-run).

#### UI izboljšave (AutoPilotCard extended)

- **Header badge**: posodobljen iz "v8.30" na "v8.31" z mode-aware subtitle ("🛡️ SAFE · LOW RISK ONLY" / "🚀 AGGRESSIVE · MEDIUM OK" / "⚠️ SUSPENDED · ANOMALY").
- **Dynamic border-color**: purple ko safe, rose ko aggressive, red ko suspended — vizualno takoj jasno kateri state je aktiven.
- **Anomaly banner** (red, eye-catching — top of card z AlertOctagon icon): prikazan ko `anomalySuspended=true`. Vsebuje reason, suspendedAt, "Razveljavi suspenzijo" button ki kliče POST `{action:'clear_anomaly'}`.
- **Aggressive active banner** (rose): prikazan ko `mode='aggressive'` in NOT suspended. Vsebuje "AGGRESSIVE MODE — višje tveganje" + thresholds (MEDIUM OK, 300€ uplift, 10/dan, 2000€/dan) + "🛡️ Nazaj v Safe" button ki kliče POST `{action:'disable_aggressive'}`.
- **Aggressive pending confirmation banner** (amber, pulse-animated): prikazan ko `aggressiveConfirmedAt != null` (after first click) in NOT yet aggressive. Vsebuje "⚠️ Aggressive mode dovoli MEDIUM confidence. Potrdi ponovno v 5 minutah." + "✅ Potrdi" button ki kliče POST `{action:'enable_aggressive'}` (second confirmation).
- **Mode selector** (2 button-a, vedno viden v config area):
  - "🛡️ Safe (LOW risk only)" — highlighted (purple) ko mode='safe', klik vrne v safe.
  - "🚀 Aggressive (MEDIUM OK)" — highlighted (rose) ko mode='aggressive', pulse-animated (amber) ko pending confirmation. Disabled ko anomaly suspended.
- **Enhanced today's stats**: mode-aware stats line "Danes: 3/10 akcij (aggressive) · 145€/2000€ budget" + hourly counter line "Zadnja ura: 2 akciji (od HH:MM:SS)" z warning "⚠️ blizu anomaly threshold (8)" ko count ≥ 6. Progress bars ostanejo.
- **Threshold comparison box** (always visible — 2-column grid Safe vs Aggressive z 6 pravili vsak: confidence, uplift, limit, budget, domain, HIGH=manual. Aktivni mode je highlighted (purple za Safe, rose za Aggressive), neaktivni je dimmed).
- **Safety info box** posodobljen: 8 pravil z mode-aware opisi (npr. "Confidence dovoljen za mode (safe: LOW; aggressive: LOW+MEDIUM; HIGH vedno ročno)").
- **Footer** prikazuje vseh 5 POST akcij: "run, config, enable_aggressive, disable_aggressive, clear_anomaly".
- **"▶️ Zaženi zdaj" button** disabled ko `anomalySuspended=true` (ne moreš zagnati run-a ko je suspended).
- **NEW lucide-react imports**: `AlertOctagon`, `ShieldAlert`, `Rocket`.
- **NEW TypeScript interfaces**: `EnableAggressiveResponse`, `DisableAggressiveResponse`, `ClearAnomalyResponse`.
- **NEW state**: `aggressivePending` (synced from `stats.config.aggressiveConfirmedAt`), `aggressiveMsg`, `togglingMode`, `clearingAnomaly`.
- **BrainSynthesisCard outer badge** posodobljen: "v8.31 Auto-pilot + v8.29 Draft Queue + v8.28 Adaptive + v8.27 Scenario + v8.26 Explain + v8.25 Accuracy + v8.24 Personal + v8.23 Validation + v8.22 Master + v8.15-v8.21 (7 Domains)".
- **brain/auto-pilot endpoint** v AI Hub listi dobi "v8.31 · AUTO+" badge (purple) (prej "v8.30 · AUTO"). brain/auto-pilot/rollback ostane "v8.30 · UNDO" (samo rollback route ni bil modified v v8.31).

#### Stats

- AI endpointi: 424 (nespremenjeno — no new endpoints, only extended existing `brain/auto-pilot`)
- Total API routes: 601 (nespremenjeno — no new routes)
- Brain category: 20 (nespremenjeno)
- Cron endpoints: 13 (nespremenjeno — `cron/auto-pilot` je bil dodan v v8.30, v v8.31 only modified internally)
- Lint: 0 errors, 2 warnings (pred-existing unused eslint-disable v db.ts — ne kritično)
- Typecheck: 0 errors (bunx tsc --noEmit)
- db:push: uspešen (6 novih Settings columns prisotnih v SQLite — verificirano preko PRAGMA table_info)
- Endpoint verification (curl tests):
  - GET `/api/ai/brain/auto-pilot` → 200 {ok:true, config:{enabled:false, mode:"safe", dailyLimit:5, dailyBudgetEUR:500, lastRunAt:..., aggressiveConfirmedAt:null, anomalySuspended:false, anomalySuspendedAt:null, anomalyReason:null, hourlyExecCount:0, hourlyWindowStart:null}, ...} ✅
  - POST `/api/ai/brain/auto-pilot` {action:'enable_aggressive'} (first call) → 200 {ok:true, confirmed:false, message:"⚠️ Aggressive mode dovoli MEDIUM confidence. Potrdi ponovno v 5 minutah.", confirmedAt:"..."} ✅
  - POST `/api/ai/brain/auto-pilot` {action:'enable_aggressive'} (second call within 5 min) → 200 {ok:true, confirmed:true, message:"✅ Aggressive mode omogočen — dovoljena MEDIUM confidence (do 300€ uplift, 10/dan, 2000€/dan budget). HIGH je še vedno vedno manual."} ✅
  - POST `/api/ai/brain/auto-pilot` {action:'disable_aggressive'} → 200 {ok:true, mode:"safe"} ✅
  - POST `/api/ai/brain/auto-pilot` {action:'clear_anomaly'} → 200 {ok:true, message:"✅ Suspenzija razveljavljena. Auto-pilot lahko ponovno deluje (hourly counter resetiran)."} ✅
  - GET `/api/ai-list` → 200 {ok:true, total:424, categories:{brain:20, ...}} ✅ (424 AI endpointov, brain 20 — nespremenjeno, no new endpoints only extended existing)

**🎯 AUTOMATION PHASE COMPLETE.** v8.30 (Safe Auto-pilot, LOW risk only) + v8.31 (Aggressive Auto-pilot + Anomaly Detection) = full auto-pilot spectrum. Next: v8.32+ (Polish — anomaly detection ML, predictive accuracy, performance optimizations).
...
### Added — Profit Per Trade Growth Maximizer + Deal Source Annual Return Maximizer + Inventory Capital Velocity Maximizer (3 funkcije)

- **AI Profit Per Trade Growth Maximizer** — `GET+POST /api/ai/profit-per-trade-growth-maximizer`
  - AI MAXIMIZES GROWTH RATE of profit per trade — not just current profit per trade but how fast
    it's growing and how to accelerate that growth (€/month growth rate, not absolute €/trade)
  - "Your profit per trade is growing +2€/month, but could grow +8€/month with these actions."
  - Different from profit-multiplier-maximizer (v8.09 which maximizes maximum profit multiplier with
    6 dimensions) — this MAXIMIZES GROWTH RATE of profit per trade (€/mo how fast it grows, not ×
    how many times)
  - Different from inventory-profit-per-day-growth-maximizer (v8.09 which maximizes growth rate of
    daily profit from inventory in %/week) — this MAXIMIZES GROWTH RATE of profit PER TRADE in
    €/month (absolute €/mo per-trade growth, not %/week daily profit)
  - Different from profit-per-day-scaling-maximizer (v8.08 which maximizes and scales daily profit
    with scalingPath) — this MAXIMIZES GROWTH RATE of profit per trade (€/mo ramp, not €/day scaling)
  - Different from profit-acceleration-maximizer (v8.05 which maximizes growth rate acceleration)
    — this MAXIMIZES GROWTH RATE of profit per trade with growthActions (IMPROVE_SOURCING/
    RAISE_PRICES/REDUCE_FEES/TARGET_PREMIUM/TIMING_OPTIMIZATION) and doublingTime in months
  - Different from revenue-per-trade-maximizer (v8.06 which maximizes top-line sell price per
    trade) — this MAXIMIZES GROWTH RATE of profit per trade (€/mo growth, not absolute sell price)
  - Different from deal-source-profit-per-trade-maximizer (v8.04 which maximizes profit per trade
    per source) — this MAXIMIZES GROWTH RATE of profit per trade across the entire portfolio with
    growthTrajectory 12-month projection and growthGrade
  - Different from profit-velocity-maximizer (v7.98 which maximizes €/day velocity) — this
    MAXIMIZES GROWTH €/mo per trade, not €/day velocity
  - Different from profit-per-euro-maximizer (v8.07 which maximizes profit per euro deployed) —
    this MAXIMIZES GROWTH of profit per trade (€/mo growth, not €/€ ratio)
  - Different from inventory-profit-per-day-maximizer (v8.02 which maximizes daily profit per
    item) — this MAXIMIZES GROWTH RATE of profit per trade (€/mo ramp, not €/day per item)
  - Queries SOLD trades (last 12m) grouped by month
  - current: monthlyAvgProfitPerTrade € [0, 10000] (avg over 12m), currentMonthlyProfitPerTrade €
    [0, 10000] (last month with data), profitPerTradeGrowthRate €/mo [-50, 100] (linear
    regression slope of monthly profit-per-trade over 12 months), profitPerTradeTrend % (current
    vs 6 months ago), profitPerTradeAcceleration €/mo² [-50, 100] (slope of last half vs first
    half — acceleration/deceleration of growth rate), totalProfit12m €, soldCount12m,
    bestMonthProfit €, worstMonthProfit €, monthsWithData
  - maximization: currentGrowthRate €/mo [-50, 100] (echoes current), maximizedGrowthRate €/mo
    [-50, 100] (optimal achievable, ≥ current, ≤ current + 50€ absolute uplift — anti-
    hallucination), growthUplift €/mo [0, 100] (improvement = maximized − current), growthActions
    (5 entries: action IMPROVE_SOURCING/RAISE_PRICES/REDUCE_FEES/TARGET_PREMIUM/TIMING_OPTIMIZATION
    with gain 3.5/2.8/1.5/4.0/2.2 €/mo, description slovenian max 200, expectedGrowthLift €/mo
    [0, 100], timeline slovenian max 100 when to implement, difficulty EASY/MEDIUM/HARD — sorted
    by expectedGrowthLift descending), growthTrajectory (12 entries: month 1-12, currentProjected-
    ProfitPerTrade € [0, 10000] profit at current growth rate linear: base + growth × month,
    maximizedProjectedProfitPerTrade € [0, 10000] profit at maximized growth rate), growthBottlenecks
    (3-5 slovenian strings max 200 each — what limits profit per trade growth), growthGrade A+/A/B/
    C/D/F (A+ if maximized ≥ 20, A ≥ 12, B ≥ 7, C ≥ 4, D ≥ 1.5, else F), doublingTime months
    [1, 120] (months to double profit per trade at maximized growth rate — = baseProfit /
    maximizedGrowthRate; if ≤ 0, set 120)
  - Anti-hallucination: growth rates [-50, 100], profits [0, 10000]
  - 6h cache (key `profit-per-trade-growth-maximizer:${currentMonth}`)
  - Deterministic fallback (12-month buckets × linear regression slope × trend × acceleration +
    5 actions × growth lift + 12-month trajectory × linear projection + 3-5 bottlenecks + grade
    + doubling time)
  - Empty-state fallback if 0 SOLD trades → grade F, doublingTime 120

- **AI Deal Source Annual Return Maximizer** — `GET+POST /api/ai/deal-source-annual-return-maximizer`
  - AI MAXIMIZES ANNUALIZED RETURN per source — converts per-source profit into annualized return
    rate for comparison with other investments (stocks, real estate, bonds)
  - "Bolha annualized return: 85%, Vinted: 42%, but could be 120% and 68% respectively."
  - Different from deal-source-capital-return-maximizer (v8.09 which maximizes capital return rate
    per source — % capital returned) — this MAXIMIZES ANNUALIZED RETURN per source (profit /
    capital × 365 / holdDays × 100, not % capital returned)
  - Different from deal-source-profit-velocity-maximizer (v8.08 which maximizes velocity of profit
    per source — €/week how fast profit accumulates) — this MAXIMIZES ANNUALIZED RETURN per source
    (% annual return comparable with stocks/real estate, not €/week)
  - Different from deal-source-roi-maximizer (v8.00 which maximizes ROI per source) — this
    MAXIMIZES ANNUALIZED RETURN (ROI × 365 / holdDays, time-weighted) with returnVsBenchmark and
    capitalEfficiencyAdvice
  - Different from deal-source-cash-flow-maximizer (v8.06 which maximizes NET CASH FLOW per
    source) — this maximizes ANNUALIZED RETURN RATE per source (% return, not € cash flow)
  - Different from deal-source-revenue-maximizer (v8.07 which maximizes total revenue per source)
    — this maximizes ANNUALIZED RETURN (profit/capital annualized, not top-line revenue)
  - Different from deal-source-profit-maximizer (v7.97 which maximizes total profit per source) —
    this maximizes ANNUALIZED RETURN RATE per source (% annual, not € profit)
  - Different from deal-source-profit-per-trade-maximizer (v8.04 which maximizes profit per trade
    €) — this maximizes ANNUALIZED RETURN per source (time-weighted %, not €/trade)
  - Different from deal-source-margin-maximizer (v8.03 which maximizes margin %) — this maximizes
    ANNUALIZED RETURN per source with returnMaximizationAction and returnVsBenchmark
  - Different from deal-source-capital-efficiency-maximizer (v8.05 which maximizes capital
    efficiency per source = profit per euro per day) — this maximizes ANNUALIZED RETURN per source
    (annual %, not profit per euro per day)
  - Different from deal-source-volume-maximizer (v8.02 which maximizes trade volume per source) —
    this maximizes ANNUALIZED RETURN per source (% annualized return, not trade volume)
  - Different from inventory-annualized-return-maximizer (v8.06 which maximizes annualized return
    on HELD inventory) — this MAXIMIZES ANNUALIZED RETURN per SOLD SOURCE (realized trades, not
    held inventory)
  - Different from profit-per-euro-maximizer (v8.07 which maximizes profit per euro deployed across
    portfolio) — this maximizes ANNUALIZED RETURN per source with benchmark comparison (stocks 8%,
    real estate 5%, bonds 3%)
  - Queries SOLD trades (last 12m) with linked Listing (for monitor.source)
  - Per source metrics: totalProfit € [-100000, 1000000] (= sum(sellPrice − sellFees − buyPrice
    − buyFees)), totalCapitalDeployed € [0, 1000000] (= sum(buyPrice + buyFees)), avgHoldDays
    [1, 730] (= avg(sellDate − buyDate)), annualizedReturn % [-100, 1000] (= (totalProfit /
    totalCapitalDeployed) × (365 / avgHoldDays) × 100), tradeCount, profitMargin % [-100, 500]
    (= totalProfit / totalCapitalDeployed × 100)
  - maximization per source: returnMaximizationAction INCREASE_PROFIT/REDUCE_CAPITAL/FASTER_TURNOVER/
    BETTER_SOURCING (gain 25/18/30/22pp), maximizedAnnualReturn % [-100, 1000] (≥ current, ≤
    current + 200pp absolute uplift — anti-hallucination), returnUplift pp [0, 500], returnMaximi-
    zationLevers (3-5 slovenian strings max 200 each — specific annualized return levers per
    source), returnVsBenchmark { stocks % = 8, realEstate % = 5, bonds % = 3, yourCurrentReturn %,
    yourMaximizedReturn %, maximizedVsStocks pp, maximizedVsRealEstate pp, maximizedVsBonds pp },
    capitalEfficiencyAdvice (slovenian max 300 — how to deploy capital more efficiently per source
    for higher annualized return)
  - portfolio: currentPortfolioAnnualReturn %, maximizedPortfolioAnnualReturn %, totalReturnUplift
    pp, sourceReturnRanking Array<{ source, displayName, currentAnnualReturn, maximizedAnnualReturn,
    rank }>, bestReturnSource
  - Anti-hallucination: returns [-100, 1000], scores [0, 100], capital [0, 1000000]
  - 6h cache (key `deal-source-annual-return-maximizer:${currentMonth}`)
  - Deterministic fallback (per-source annualized return = margin × cycles per year + action ×
    uplift + 3-5 levers per source + returnVsBenchmark stocks/real estate/bonds + capital
    efficiency advice + portfolio ranking + best source)
  - Empty-state fallback if 0 SOLD trades

- **AI Inventory Capital Velocity Maximizer** — `GET+POST /api/ai/inventory-capital-velocity-maximizer`
  - AI MAXIMIZES VELOCITY of capital through inventory — how fast capital cycles from investment
    to return (cycle time + cycles per year, not €/day, not % return)
  - "Your capital cycles every 28 days (13×/year), but could cycle every 18 days (20×/year) with
    these actions."
  - Different from inventory-profit-per-day-growth-maximizer (v8.09 which maximizes growth rate of
    daily profit from inventory in %/week) — this MAXIMIZES CAPITAL VELOCITY (how many cycles/year,
    not %/week growth)
  - Different from profit-per-day-scaling-maximizer (v8.08 which maximizes and scales daily profit
    with scalingPath) — this MAXIMIZES CAPITAL VELOCITY (cycle time + cycles per year, not €/day
    scaling)
  - Different from inventory-turnover-yield-maximizer (v8.05 which maximizes yield with yieldCurve)
    — this MAXIMIZES CAPITAL VELOCITY with capitalMultiplierEffect (how many times capital
    multiplies in one year)
  - Different from inventory-roi-maximizer-pro (v7.99 which maximizes ROI per item) — this
    MAXIMIZES CAPITAL VELOCITY across the entire inventory (avg cycle time, not ROI per item)
  - Different from inventory-capital-efficiency-maximizer (v8.01 which maximizes capital
    efficiency per item) — this MAXIMIZES CAPITAL VELOCITY with velocityProjection and velocityGrade
    (not capital efficiency per item)
  - Different from inventory-cash-yield-maximizer (v8.04 which maximizes annualized cash yield) —
    this MAXIMIZES CAPITAL VELOCITY (cycles per year, not cash yield)
  - Different from inventory-profit-per-day-maximizer (v8.02 which maximizes daily profit per
    item) — this MAXIMIZES CAPITAL VELOCITY (how many times capital cycles, not €/day per item)
  - Different from inventory-annualized-return-maximizer (v8.06 which maximizes annualized % return
    on held inventory) — this MAXIMIZES CAPITAL VELOCITY with maximizedCycleTime and maximizedCycles-
    PerYear (not % return)
  - Different from inventory-capital-return-maximizer (v8.07 which maximizes capital return OF
    inventory) — this MAXIMIZES CAPITAL VELOCITY (how many times it cycles, not % returned)
  - Different from inventory-return-on-capital-maximizer (v8.08 which maximizes return ON capital
    for HELD inventory) — this MAXIMIZES CAPITAL VELOCITY with velocityMaximizationActions (FASTER_
    PRICING/BETTER_LISTINGS/CROSS_POSTING/REDUCE_HOLD_TIME/OPTIMIZE_SOURCING) and capitalMultiplierEffect
  - Different from profit-velocity-maximizer (v7.98 which maximizes €/day velocity) — this
    MAXIMIZES CAPITAL VELOCITY (cycle time + cycles/year, not €/day)
  - Different from profit-multiplier-maximizer (v8.09 which maximizes maximum profit multiplier
    with 6 dimensions) — this MAXIMIZES CAPITAL VELOCITY (capital cycling, not profit multiplier)
  - Queries SOLD trades (last 12m) + HELD trades (inventory pipeline) in parallel
  - current: avgCapitalCycleTime days [1, 365] (= avg(sellDate − buyDate) over SOLD 12m),
    capitalCyclesPerYear [0, 100] (= 365 / avgCapitalCycleTime), capitalVelocityScore [0, 100]
    (cycle time + cycles per year + profit margin combined), currentCapitalMultiplier × [0, 100]
    (= cycles × margin / 100), totalCapitalDeployed € [0, 1000000] (= SOLD 12m + HELD),
    heldCapital €, soldCount12m, heldCount, avgProfitMargin % [-100, 500] (= totalProfit /
    soldCapital × 100)
  - maximization: maximizedCycleTime days [1, 365] (optimal cycle time, shorter = faster — ≤
    current.avgCapitalCycleTime, anti-hallucination), maximizedCyclesPerYear [0, 100] (= 365 /
    maximizedCycleTime, ≥ current.capitalCyclesPerYear), velocityUplift cycles/year [0, 50]
    (improvement = maximized − current cycles), velocityMaximizationActions (5 entries: action
    FASTER_PRICING/BETTER_LISTINGS/CROSS_POSTING/REDUCE_HOLD_TIME/OPTIMIZE_SOURCING with gain
    2.5/1.8/3.2/4.0/1.5 cycles/yr, description slovenian max 200, expectedVelocityGain cycles/
    year [0, 50], priority HIGH/MEDIUM/LOW — sorted by expectedVelocityGain descending),
    velocityProjection (6 entries: month 1-6, currentCyclesPerYear [0, 100] constant,
    maximizedCyclesPerYear [0, 100] linear ramp: 1m=17%, 3m=50%, 6m=100% adoption of maximized
    cycles), velocityBottlenecks (3-5 slovenian strings max 200 each — what slows capital
    cycling), velocityGrade A+/A/B/C/D/F (A+ if maximized cycles ≥ 25, A ≥ 18, B ≥ 12, C ≥ 8,
    D ≥ 5, else F), capitalMultiplierEffect × [0, 100] (= maximizedCyclesPerYear × avgProfitMargin
    / 100 — how many times capital multiplies per year at maximized velocity)
  - Anti-hallucination: cycle times [1, 365], cycles [0, 100], scores [0, 100]
  - 6h cache (key `inventory-capital-velocity-maximizer:${currentMonth}`)
  - Deterministic fallback (cycle time compression factor × maximized cycle + 5 actions × velocity
    gain + 6-month projection × linear ramp + 3-5 bottlenecks + grade + capital multiplier effect)
  - Empty-state fallback if 0 SOLD and 0 HELD trades → grade F

## [8.09.0] - 2026-08-22

### Added — Profit Multiplier Maximizer + Deal Source Capital Return Maximizer + Inventory Profit Per Day Growth Maximizer (3 funkcije)

- **AI Profit Multiplier Maximizer** — `GET+POST /api/ai/profit-multiplier-maximizer`
  - AI MAXIMIZES MAXIMUM PROFIT MULTIPLIER — how many times current profit can be multiplied
    by combining ALL profit optimization dimensions into a single cumulative multiplier
    (compounding product of 6 dimensions: pricing × volume × sourcing × turnover × channel × efficiency)
  - "Your current profit is 2000€/mo. With all optimizations, it could be 7.2× = 14,400€/mo."
  - Different from profit-per-day-scaling-maximizer (v8.08 which maximizes and scales DAILY PROFIT
    with scalingPath phases) — this MAXIMIZES PROFIT MULTIPLIER (cumulative product of all 6
    dimensions, not daily profit scaling)
  - Different from profit-per-euro-maximizer (v8.07 which maximizes profit per euro deployed —
    capital efficiency ratio) — this MAXIMIZES PROFIT MULTIPLIER (× how many times profit, not
    €/€ ratio)
  - Different from revenue-per-trade-maximizer (v8.06 which maximizes top-line sell price per
    trade) — this MAXIMIZES PROFIT MULTIPLIER with 6 dimension multipliers and cumulativeMaxMultiplier
  - Different from deal-source-cash-flow-maximizer (v8.06 which maximizes cash flow per source)
    — this MAXIMIZES PROFIT MULTIPLIER across the entire portfolio with multiplierGrade and
    multiplierProjection
  - Different from inventory-annualized-return-maximizer (v8.06 which maximizes annualized %
    return on held inventory) — this maximizes PROFIT MULTIPLIER (× how many times, not % return)
  - Different from inventory-capital-return-maximizer (v8.07 which maximizes return OF capital)
    — this maximizes PROFIT MULTIPLIER (return ON profit scaled, not capital returned)
  - Different from profit-scale-engine (v7.96 which scales profit with growth engine) — this
    MAXIMIZES PROFIT MULTIPLIER with 6 dimension multipliers (pricing/volume/sourcing/turnover/
    channel/efficiency) and cumulativeMaxMultiplier
  - Different from profit-velocity-maximizer (v7.98 which maximizes €/day velocity) — this
    maximizes PROFIT MULTIPLIER (cumulative ×, not €/day velocity)
  - Different from profit-acceleration-maximizer (v8.05 which maximizes growth rate
    acceleration) — this MAXIMIZES PROFIT MULTIPLIER with multiplierGrade and topMultiplierActions
  - Different from profit-multiplier-engine (v7.96 which has multiplier-like scoring) — this
    MAXIMIZES PROFIT MULTIPLIER with cumulativeMaxMultiplier (compounding product of all 6
    dimensions) and multiplierProjection (3/6/12 month profit at maximized multiplier)
  - Queries SOLD trades (last 12m) + HELD trades in parallel
  - current: monthlyProfit €/mo (= totalProfit12m / 12), avgROI % (= totalProfit12m / soldCapital ×
    100), avgHoldDays, tradeVolume trades/12m, capitalDeployed € (SOLD 12m + HELD), totalProfit12m €,
    soldCount12m, heldCount
  - maximization: pricingMultiplier [1.0, 2.0] (max profit gain from optimal pricing — AI up to
    1.6 default), volumeMultiplier [1.0, 3.0] (max profit gain from increased trade volume — AI up
    to 3.0 if tradeVolume < 12), sourcingMultiplier [1.0, 2.0] (max profit gain from better buy
    prices — AI up to 1.7 default), turnoverMultiplier [1.0, 2.5] (max profit gain from faster
    turnover — AI up to 2.5 if avgHoldDays > 90), channelMultiplier [1.0, 1.5] (max profit gain
    from optimal platforms — AI up to 1.4 default), efficiencyMultiplier [1.0, 1.5] (max profit
    gain from reduced fees — AI up to 1.3 default), cumulativeMaxMultiplier [1.0, 50.0] (product
    of all 6 multipliers — compounding), maximizedMonthlyProfit € [0, 200000] (= current.monthlyProfit
    × cumulativeMaxMultiplier), maximizationBreakdown (6 entries: dimension PRICING/VOLUME/
    SOURCING/TURNOVER/CHANNEL/EFFICIENCY, multiplier [1.0, 3.0], contribution % [0, 100] share of
    cumulative lift, rationale slovenian max 200), multiplierGrade A+/A/B/C/D/F (A+ if cumulative
    ≥ 15.0, A ≥ 8.0, B ≥ 4.0, C ≥ 2.0, D ≥ 1.2, else F), topMultiplierActions (6-8 entries:
    dimension, action slovenian max 200, expectedMultiplierLift [1.0, 3.0], priority HIGH/MEDIUM/
    LOW sorted by lift descending), multiplierProjection (3 entries: months 3/6/12, projectedProfit
    € [0, 200000] linear ramp 3m=25%, 6m=50%, 12m=100% adoption of cumulative multiplier)
  - Anti-hallucination: multipliers [1.0, 3.0] each, cumulative [1.0, 50.0], profits [0, 200000]
  - 6h cache (key `profit-multiplier-maximizer:${currentMonth}`)
  - Deterministic fallback (6 dimension multipliers × current state heuristics × cumulative product
    × breakdown contribution × top actions sorted by lift × 3/6/12 month projection + grade)
  - Empty-state fallback if 0 SOLD and HELD trades → grade F

- **AI Deal Source Capital Return Maximizer** — `GET+POST /api/ai/deal-source-capital-return-maximizer`
  - AI MAXIMIZES CAPITAL RETURN per source — how much of the invested capital comes back from each
    source (% returned / invested, not € profit, not € cash flow)
  - "Bolha returns 85% of invested capital, Vinted returns 72%. Optimal: 95% Bolha + 88% Vinted
    with these actions."
  - Different from deal-source-profit-velocity-maximizer (v8.08 which maximizes VELOCITY of profit
    per source — €/week how fast profit accumulates) — this MAXIMIZES CAPITAL RETURN per source
    (% invested capital that returns, not €/week profit velocity)
  - Different from deal-source-cash-flow-maximizer (v8.06 which maximizes NET CASH FLOW per source
    after fees + carrying costs) — this maximizes CAPITAL RETURN RATE per source (% capital
    returned, not € cash flow)
  - Different from deal-source-revenue-maximizer (v8.07 which maximizes total revenue per source)
    — this maximizes CAPITAL RETURN (returned/invested, not top-line revenue)
  - Different from deal-source-profit-maximizer (v7.97 which maximizes total profit per source) —
    this maximizes CAPITAL RETURN RATE per source (% capital that returns, not € profit)
  - Different from deal-source-profit-per-trade-maximizer (v8.04 which maximizes profit per trade
    €) — this maximizes CAPITAL RETURN per source (% returned, not €/trade)
  - Different from deal-source-margin-maximizer (v8.03 which maximizes margin %) — this maximizes
    CAPITAL RETURN RATE per source with returnMaximizationAction and capitalRecyclingSpeed
  - Different from deal-source-roi-maximizer (v8.00 which maximizes ROI per source) — this
    maximizes CAPITAL RETURN (returned/invested, not profit/cost)
  - Different from deal-source-capital-efficiency-maximizer (v8.05 which maximizes capital
    efficiency per source = profit per euro per day) — this maximizes CAPITAL RETURN RATE per
    source (% capital returned, not profit per euro per day)
  - Different from deal-source-volume-maximizer (v8.02 which maximizes trade volume per source)
    — this maximizes CAPITAL RETURN per source (% capital that returns, not trade volume)
  - Different from inventory-capital-return-maximizer (v8.07 which maximizes capital return OF
    inventory portfolio) — this maximizes CAPITAL RETURN per SOURCE with sourceReturnRanking
    and returnAtRiskCapital
  - Different from profit-per-euro-maximizer (v8.07 which maximizes profit per euro deployed
    across portfolio) — this maximizes CAPITAL RETURN per source (% returned, not €/€ profit)
  - Queries SOLD trades (last 12m) with linked Listing (for monitor.source)
  - Per source metrics: capitalInvested € (= sum(buyPrice + buyFees)), capitalReturned € (=
    sum(sellPrice − sellFees)), returnRate % [0, 200] (= capitalReturned / capitalInvested × 100),
    avgReturnTime days [1, 730] (= avg(sellDate − buyDate)), returnEfficiency [0, 100] (returnRate ×
    speed combined), tradeCount
  - maximization per source: returnMaximizationAction IMPROVE_SELL_PRICE/REDUCE_FEES/FASTER_SALE/
    BETTER_SOURCING (uplift 12/5/8/15pp), maximizedReturnRate % [0, 200] (≥ current, ≤ current + 30pp
    absolute uplift — anti-hallucination), returnUplift pp [0, 200], returnMaximizationLevers (3-5
    slovenian strings max 200 each — specific capital return levers per source),
    capitalRecyclingSpeed [0, 100] (how fast capital returns for reinvestment — combination of
    returnRate × speed × action boost), returnAtRiskCapital € [0, 1000000] (capital that may not
    return — items selling below cost in this source)
  - portfolio: totalCurrentReturnRate %, totalMaximizedReturnRate %, totalReturnUplift pp,
    totalCapitalAtRisk €, sourceReturnRanking Array<{ source, displayName, currentReturnRate,
    maximizedReturnRate, rank }>, bestReturnSource
  - Anti-hallucination: return rates [0, 200], scores [0, 100], capital [0, 1000000]
  - 6h cache (key `deal-source-capital-return-maximizer:${currentMonth}`)
  - Deterministic fallback (per-source return rate + efficiency + action × uplift + 3-5 levers per
    source + recycling speed + capital at risk + portfolio ranking + best source)
  - Empty-state fallback if 0 SOLD trades

- **AI Inventory Profit Per Day Growth Maximizer** — `GET+POST /api/ai/inventory-profit-per-day-growth-maximizer`
  - AI MAXIMIZES GROWTH RATE of daily profit from inventory — not just current daily profit, but
    how fast it's growing and how to accelerate that growth (%/week growth rate, not absolute €/day)
  - "Your daily profit growth is +2%/week, but could be +5%/week with these 4 actions."
  - Different from profit-per-day-scaling-maximizer (v8.08 which maximizes and scales DAILY PROFIT
    with scalingPath phases) — this MAXIMIZES GROWTH RATE of daily profit (%/week how fast it
    grows, not absolute €/day)
  - Different from profit-acceleration-maximizer (v8.05 which maximizes growth rate acceleration)
    — this MAXIMIZES GROWTH RATE of daily profit from INVENTORY with growthAccelerationActions
    and doublingTime (not just acceleration)
  - Different from profit-multiplier-maximizer (v8.09 which maximizes maximum profit multiplier
    with 6 dimensions) — this MAXIMIZES GROWTH RATE (how much %/week, not × how many times)
  - Different from inventory-cash-yield-maximizer (v8.04 which maximizes annualized cash yield)
    — this maximizes GROWTH RATE of daily profit (not annualized yield)
  - Different from inventory-turnover-yield-maximizer (v8.05 which maximizes yield with
    yieldCurve) — this MAXIMIZES GROWTH RATE with growthTrajectory and doublingTime
  - Different from inventory-yield-maximizer (v8.03 which maximizes yield % per item) — this
    MAXIMIZES GROWTH RATE of daily profit with growthGrade and growthBottlenecks
  - Different from inventory-roi-maximizer-pro (v7.99 which maximizes ROI per item) — this
    MAXIMIZES GROWTH RATE of daily profit from inventory (not ROI per item)
  - Different from inventory-capital-efficiency-maximizer (v8.01 which maximizes capital
    efficiency per item) — this MAXIMIZES GROWTH RATE of daily profit with growthTrajectory (not
    capital efficiency per item)
  - Different from inventory-capital-return-maximizer (v8.07 which maximizes capital return OF
    inventory) — this MAXIMIZES GROWTH RATE of daily profit (return ON profit growth, not
    capital returned)
  - Different from inventory-profit-per-day-maximizer (v8.02 which maximizes daily profit per
    item) — this MAXIMIZES GROWTH RATE of daily profit (how much %/week grows, not absolute
    €/day per item)
  - Different from inventory-annualized-return-maximizer (v8.06 which maximizes annualized %
    return on held inventory) — this MAXIMIZES GROWTH RATE of daily profit with doublingTime
    (not annualized return)
  - Queries SOLD trades (last 12m) grouped by week + HELD trades (inventory pipeline)
  - current: weeklyProfitGrowthRate %/week [-50, 100] (linear regression slope of weekly profit
    over 52 weeks), dailyProfitGrowthRate %/day [-50, 100] (= weekly / 7), profitGrowthAcceleration
    pp [-50, 100] (slope of last half vs first half), profitGrowthVolatility [0, 100] (coefficient
    of variation of weekly profit), currentDailyProfit €/day [0, 10000] (= totalProfit12m / 365),
    totalProfit12m €, soldCount12m, heldCount, heldCapital €
  - maximization: currentGrowthRate %/week [-50, 100] (echoes current), maximizedGrowthRate
    %/week [-50, 100] (optimal achievable, ≥ current, ≤ current × 2.5 or +50pp absolute uplift —
    anti-hallucination), growthUplift pp [0, 100] (improvement = maximized − current),
    growthAccelerationActions (4-6 entries: action slovenian max 200 — specific action to
    accelerate growth rate, expectedGrowthLift pp [0, 100] how much pp will be added to growth rate,
    timeline slovenian max 100 when to implement, difficulty EASY/MEDIUM/HARD), growthTrajectory
    (8 entries: week 1-8, currentProjectedProfit €/day [0, 10000] profit at current growth rate
    compound: base × (1 + rate)^week, maximizedProjectedProfit €/day [0, 10000] profit at maximized
    growth rate), growthBottlenecks (3-5 slovenian strings max 200 each — what limits growth rate
    acceleration), growthSustainability [0, 100] (whether current growth is sustainable —
    combination of volatility × inventory × trade frequency × acceleration), growthGrade A+/A/B/C/
    D/F (A+ if maximized × sustainability/100 ≥ 15, A ≥ 8, B ≥ 4, C ≥ 2, D ≥ 0.5, else F),
    doublingTime days [1, 3650] (days to double daily profit at maximized growth rate — =
    ln(2) / ln(1 + g/100) / 7 × 7 = ln(2) / ln(1 + g/100) days where g = maximizedGrowthRate %)
  - Anti-hallucination: growth rates [-50, 100], profits [0, 10000]
  - 6h cache (key `inventory-profit-per-day-growth-maximizer:${currentMonth}`)
  - Deterministic fallback (52-week profit buckets × linear regression slope × acceleration ×
    volatility + 4-6 actions × growth lift + 8-week trajectory × compound growth + 3-5
    bottlenecks + sustainability score + grade + doubling time)
  - Empty-state fallback if 0 SOLD and HELD trades → grade F
...
### Added — AI Revenue Growth Maximizer & AI Deal Profit Margin Enhancer Pro & AI Inventory Capital Efficiency Maximizer (3 funkcije — REVENUE GROWTH & MARGIN ENHANCEMENT & CAPITAL EFFICIENCY focus)

- **AI Revenue Growth Maximizer** — `GET+POST /api/ai/revenue-growth-maximizer`
  - AI maksimizira REVENUE GROWTH (top-line revenue, NE samo profit).
    "Your revenue is 5000€/mo but could be 12,000€/mo with 3 expansion
    actions." Fokus na total revenue growth, expansion v nove kategorije
    in scaling trade volume. Razlika od profit-multiplier-engine (v8.00
    ki MULTIPLICIRA profit z 8 levers) — ta MAXIMIZIRA REVENUE
    (top-line sales), ne profit. Razlika od deal-source-roi-maximizer
    (v8.00 ki maksimizira ROI per source) — ta fokusira na REVENUE
    GROWTH rate in category expansion (ne ROI %). Razlika od
    inventory-turnover-profit-maximizer (v8.00 ki maksimizira
    turnover-profit balance) — ta maksimizira TOTAL REVENUE preko
    volume scaling in category expansion. Razlika od capital-growth-
    maximizer (v7.99 ki maksimizira capital growth) — ta maksimizira
    REVENUE (top-line), ne capital. Razlika od revenue-stream-
    optimizer (v7.94 ki optimira revenue streams) — ta MAXIMIZIRA
    revenue z growth actions in category expansion + 3m/6m/12m
    projection + timeToDoubleRevenue. Razlika od profit-maximizer-pro
    (v7.94 ki maksimizira profit z 7 levers) — ta maksimizira REVENUE
    (ne profit) — different focus (top-line vs bottom-line). Razlika od
    deal-source-profit-maximizer (v7.97 ki maksimizira profit per
    source) — ta daje UNIFIED revenue growth engine (ne per-source
    profit). "Current: 5000€/mo revenue, growth 8%/mo, 12 trades/mo,
    avgRevenuePerTrade 417€. Maximization: 12,000€/mo (2.4x multiplier),
    3 growth actions (expand to mobile.de +3500€, increase trade
    volume to 24/mo +2500€, raise prices +1000€). Projection 3m:
    16500€, 6m: 24000€, 12m: 58000€. Category expansion: electronics
    (+2800€/mo), automotive (+1500€/mo). Volume scaling: 12→24
    trades/mo (2x). Diversification: Bolha 70% → Bolha 40% + Vinted
    30% + mobile.de 30%. Grade A. Time to double revenue: 87 days."
  - current: { currentMonthlyRevenue € [0, 100000] (= avg monthly
    revenue last 6m, fallback 12m), revenueGrowthRate % [0, 200]
    (= (last3m avg − prev3m avg) / prev3m avg × 100), avgRevenuePerTrade
    € [0, 100000] (= revenue12m / count12m), tradeVolumePerMonth [0,
    1000] (= count12m / 12), revenueStreams Array<{ source, percentage
    % [0, 100], revenue € }> (top 6 sources by revenue share) }
  - maximization: { maximizedMonthlyRevenue € [0, 100000] (= current
    + totalGain), revenueGrowthMultiplier [1.0, 5.0] (anti-hallucination
    — CLAMPED, = maximized / current), growthActions 4-6 [{ action
    max 200 (slovenski), expectedRevenueGain € [0, 50000], priority
    HIGH/MEDIUM/LOW, difficulty EASY/MEDIUM/HARD }], revenueProjection3m
    € [0, 300000], revenueProjection6m € [0, 600000], revenueProjection12m
    € [0, 1200000] (z growth rate compounding), categoryExpansionOpportunities
    3-5 [{ category max 50, potentialRevenue € [0, 50000], reasoning
    max 200 (slovenski) }], volumeScalingPlan max 400 (slovenski),
    revenueDiversificationStrategy max 400 (slovenski), revenueGrowthGrade
    A+/A/B/C/D/F (≥3.5 A+, ≥2.5 A, ≥1.8 B, ≥1.4 C, ≥1.15 D, else F),
    timeToDoubleRevenue dni [1, 3650] (= ln(2) / ln(1+dailyGrowth)) }
  - Compute: query SOLD 12m z linked Listing.monitor.source, per trade
    compute revenue (sellPrice − sellFees), aggregate per month + per
    source; currentMonthlyRevenue = avg last 6m (fallback 12m),
    revenueGrowthRate = (last3m avg − prev3m avg) / prev3m avg × 100,
    avgRevenuePerTrade, tradeVolumePerMonth, revenueStreams (top 6
    sources by revenue share); build 4 deterministic growth actions
    (category expansion +70% of monthly, volume scaling +50%, premium
    pricing +15% of revenuePerTrade×volume, new source +25/40% based
    on top source concentration), maximized = current + totalGain,
    multiplier = maximized/current [1.0, 5.0] CLAMPED, projections
    (3m/6m/12m z growth rate compounding), 3 category expansions
    (electronics +55%, automotive +35%, luxury fashion +30% of monthly),
    volumeScalingPlan z working capital calc, revenueDiversificationStrategy
    (top source ≤40% target + cross-post 3 platforms), grade iz multiplier,
    timeToDoubleRevenue = ln(2)/ln(1+dailyGrowth).
  - AI-enhanced z grounding prompt (soldCount12m + 6m + 3m + revenue12m
    + 6m + 3m + uniqueSources + current + deterministicMaximization +
    caps) + anti-hallucination (revenueGrowthMultiplier CLAMPED [1.0,
    5.0], gain [0, 50000], days [1, 3650], percentage [0, 100], enums
    validirana HIGH/MEDIUM/LOW in EASY/MEDIUM/HARD in A+/A/B/C/D/F,
    string length limits — action max 200, reasoning max 200, plan
    max 400, strategy max 400, summary max 400) + 6h cache (key
    `revenue-growth-maximizer:${currentMonth}` — YYYY-MM, invalidira
    monthly) + deterministic fallback (4 growth actions hevristika,
    multiplier iz gain ratio, grade iz multiplier, projections z
    compounding, 3 fixed category expansions).
  - GET+POST (handleRevenueGrowthMaximizer shared function — AI Hub
    runner kompatibilnost). Empty-state fallback če 0 SOLD trades →
    "Ni SOLD trgovin v zadnjih 12 mesecih" z aiUsed=false + empty
    current z 0 + empty maximization z grade F + timeToDoubleRevenue 0.

- **AI Deal Profit Margin Enhancer Pro** — `GET+POST /api/ai/deal-profit-margin-enhancer-pro`
  - AI ENHANCE-A profit margin na vsakem HELD item-u preko specifičnih
    FIZIČNIH/LISTING enhancement akcij (refurbishment, repositioning,
    premium packaging, better photos, improved descriptions,
    certification). Gre BEYOND pricing optimization — fokus na
    physical/listing improvements ki dejansko povečajo PERCEIVED
    VALUE in consequently margin. Razlika od profit-multiplier-engine
    (v8.00 ki multiplicira profit z 8 levers) — ta ENHANCE-A MARGIN
    per item z specifičnimi fizikalnimi akcijami (ne global profit
    multiplication). Razlika od inventory-roi-maximizer-pro (v7.99
    ki maksimizira ROI per item z abstract strategies) — ta fokusira
    na MARGIN ENHANCEMENT z enhancment ROI (marginGain / cost). Razlika
    od deal-profit-accelerator-pro (v7.99 ki accelera profit per item)
    — ta ENHANCE-A margin preko physical/listing improvements (ne
    time/profit acceleration). Razlika od deal-source-roi-maximizer
    (v8.00 ki maksimizira ROI per source) — ta daje PER-ITEM margin
    enhancement z enhancementROI. Razlika od refurb-roi-calculator
    (ki calc-a refurb ROI) — ta daje FULL margin enhancement portfolio
    z 8 actions. "iPhone 13: buyPrice 450€, estValue 580€, currentMargin
    28%. Enhancement: REFURBISH (cost 25€, marginAfter 35%, +7pp, ROI
    2.8, 3d, 85% success). REPHOTOGRAPH (cost 5€, marginAfter 32%, +4pp,
    ROI 8.0, 1d, 90% success). CERTIFY_AUTHENTICITY (cost 15€, marginAfter
    33%, +5pp, ROI 3.3, 7d, 80% success). Portfolio: totalEnhancementCost
    145€, totalMarginEnhancement +180€, ROI 1.24x. Quick wins:
    REPHOTOGRAPH (EASY, ROI 8.0), BUNDLE (EASY, ROI 5.2). Priority
    ranking: 1. PS5 REFURBISH (ROI 4.5), 2. iPhone 13 REPHOTOGRAPH
    (ROI 8.0)..."
  - items: Array<{ tradeId (MORA match-at heldItems — anti-hallucination),
    title, category, buyPrice € [0, 100000], aiEstimatedValue € | null,
    currentMargin % [-50, 200] (= (estValue − cost − 5% fees) / cost ×
    100), marginEnhancementPotential 0-100, enhancementDifficulty
    EASY/MEDIUM/HARD (EASY: margin≥1.4x in daysHeld<21, MEDIUM: ≥1.2x
    in <60, HARD: <1.1x ali ≥90d), enhancementAction
    REFURBISH/REPOSITION/REPHOTOGRAPH/REWRITE_DESCRIPTION/REPRICE_PREMIUM/BUNDLE_WITH_ACCESSORY/CERTIFY_AUTHENTICITY/NONE_NEEDED
    (8 akcij), estimatedCost € [0, 500] (koliko € stane enhancement —
    material, čas), estimatedMarginAfter % [-50, 200] (MORA biti ≥
    currentMargin — anti-hallucination), marginEnhancement % [0, 50]
    (= marginAfter − current), enhancementROI [0, 50] (= marginGainEur /
    estimatedCost), timeToImplement dni [0, 365], enhancementSteps 3-6
    stringov (max 200 vsak, slovenski — specifični koraki),
    successProbability % [0, 100] }>
  - portfolio: { totalEnhancementCost € (sum), totalMarginEnhancement €
    (sum (marginEnhancement/100 × buyPrice)), portfolioEnhancementROI [0,
    50] (= totalMarginEnhancement / totalEnhancementCost), enhancementPriority
    top 10 ranked by enhancementROI Array<{ tradeId, title, enhancementROI,
    rank }>, quickEnhancements top 5 EASY + high ROI Array<{ tradeId, title,
    action, roi }> }
  - Compute: query HELD z linked Listing (aiEstimatedValue, price,
    aiScore, dealScore), per item compute estValue (aiEst > price >
    buyPrice×1.1 fallback), cost (buyPrice + buyFees), currentProfit
    (estValue − cost − 5% fees), currentMargin (profit/cost × 100),
    marginMultiple (estValue/cost); decideDifficulty (EASY: margin≥1.4x
    in daysHeld<21, MEDIUM: ≥1.2x in <60, HARD: <1.1x ali ≥90d);
    decideEnhancementAction (luxury→CERTIFY_AUTHENTICITY, high dealScore+stale
    →REFURBISH, low value+low margin→BUNDLE_WITH_ACCESSORY, low dealScore
    →REPOSITION, default→REPHOTOGRAPH); ACTION_PROFILES (cost/days/success
    per action); marginEnhancementPotential = marginRoom × dealScoreFactor
    × difficultyFactor; estimatedMarginAfter = currentMargin + actionLift
    × potentialFactor (CLAMPED ≤ currentMargin+50pp anti-hallucination);
    marginEnhancement = marginAfter − current; enhancementROI =
    marginGainEur / estimatedCost; enhancementSteps (per-action 3-6
    korakov slovenski).
  - AI-enhanced z grounding prompt (top 40 items by enhancementPotential +
    caps + deterministic baseline) + anti-hallucination (tradeId MORA
    match-at heldItems — skip unknown, estimatedMarginAfter MORA ≥
    currentMargin, estimatedCost [0, 500], enhancementROI [0, 50],
    marginEnhancement [0, 50], successProbability [0, 100], enums
    validirana 8 actions in EASY/MEDIUM/HARD, string length limits —
    step max 200, summary max 400) + 6h cache (key
    `deal-profit-margin-enhancer-pro:${JSON.stringify(heldItemIds)}` —
    invalidira ko held inventory se spremeni) + deterministic fallback
    (action iz hevristika, cost/days/success iz ACTION_PROFILES, ROI iz
    marginGain/cost, steps iz per-action templates).
  - GET+POST (handleDealProfitMarginEnhancerPro shared function — AI Hub
    runner kompatibilnost). Empty-state fallback če 0 HELD trades →
    "Ni HELD trgovin v inventarju" z aiUsed=false + empty items + empty
    portfolio z 0 + empty enhancementPriority + empty quickEnhancements.

- **AI Inventory Capital Efficiency Maximizer** — `GET+POST /api/ai/inventory-capital-efficiency-maximizer`
  - AI MAXIMIZIRA CAPITAL EFFICIENCY — kako eficientno vsak evro kapitala
    deluje. Identificira kapital ujet v low-efficiency item-ih in
    priporoča reallokacijo v high-efficiency priložnosti. "Your capital
    efficiency is 1.8x, but could be 3.2x if you shift 1500€ from slow
    items to fast-moving categories." Razlika od capital-growth-maximizer
    (v7.99 ki maksimizira compounding capital growth) — ta MAXIMIZIRA
    capital efficiency (kapital deployment efficiency per item, ne
    compounding growth rate). Razlika od capital-allocation-optimizer
    (ki alokira kapital po kategorijah) — ta daje PER-ITEM capital
    efficiency analizo + reallocation plan iz low → high efficiency
    items. Razlika od inventory-roi-maximizer-pro (v7.99 ki maksimizira
    ROI per item) — ta maksimizira CAPITAL EFFICIENCY (value multiplier
    + profitPerEuroDeployed + efficiencyLossPerDay), ne samo ROI %.
    Razlika od inventory-cash-conversion-maximizer (v7.98 ki maksimizira
    cash conversion) — ta maksimizira CAPITAL VELOCITY preko reallocation,
    ne cash conversion speed. Razlika od deal-profit-accelerator-pro
    (v7.99 ki accelera profit per item) — ta daje CAPITAL REALLOCATION
    plan iz low → high efficiency items z efficiencyUplift forecast.
    Razlika od profit-multiplier-engine (v8.00 ki multiplicira profit z 8
    levers) — ta fokusira na CAPITAL EFFICIENCY per item + reallocation
    strategy. Razlika od inventory-turnover-profit-maximizer (v8.00 ki
    maksimizira turnover-profit balance) — ta maksimizira CAPITAL
    DEPLOYMENT efficiency. "Current: 8,500€ deployed, portfolio efficiency
    1.8x (1€ → 1.8€ value), avg profit/euro 0.45€, avg trapped 32 days,
    score 62/100 (grade C). Item analysis: iPhone 13 2.4x (high), PS5 1.5x
    (medium), old TV 0.8x (LOW — trapped 65d). Maximization: liquidate
    TV + 2 other low items (1500€ freed), reinvest in electronics (high
    velocity, 2.8x projected). Projected efficiency 3.2x (+1.4 uplift),
    expected profit uplift 680€. Grade B → A. Capital velocity: cycle
    cash every 18d instead of 32d."
  - current: { totalCapitalDeployed € [0, 100000], portfolioCapitalEfficiency
    [0, 10] (weighted avg estValue/capitalDeployed), avgProfitPerEuroDeployed
    [-2, 5], avgCapitalTrappedDuration dni [0, 730], portfolioEfficiencyScore
    0-100 }
  - perItem: Array<{ tradeId, title, capitalDeployed € [0, 100000],
    capitalEfficiency [0, 10] (estValue/capitalDeployed), profitPerEuroDeployed
    [-2, 5] (profit/capitalDeployed), efficiencyScore 0-100 (50% capitalEff
    + 30% profitPerEuro + 20% speed − trappedDuration penalty + historical
    bonus), capitalTrappedDuration dni [0, 730], efficiencyLossPerDay [0, 5]
    (1.5%/mo opportunity cost / capitalDeployed) }>
  - maximization: { lowEfficiencyItems Array<{ tradeId (MORA match-at
    topItems), title, efficiencyScore 0-100, recommendedAction max 200
    (slovenski) }> (filter efficiencyScore < 40), highEfficiencyOpportunities
    3-5 Array<{ category max 50, expectedEfficiency [1.0, 10.0], expectedROI
    % [-50, 300] }>, capitalReallocationPlan Array<{ fromTradeId (MORA
    match-at topItems), toCategory max 50, amount € [0, 100000], reasoning
    max 400 (slovenski) }>, projectedEfficiency [0, 10], efficiencyUplift
    [0, 10] (= projected − current), capitalVelocityOptimization max 400
    (slovenski), efficiencyGrade A+/A/B/C/D/F (A+ če score≥85 in eff≥2.5,
    A ≥75/2.0, B ≥60/1.5, C ≥45/1.2, D ≥30/1.0, else F), totalCapitalToReallocate
    € [0, 100000], expectedProfitUplift € [0, 50000] (= reallocAmount ×
    (reallocatedEff − currentEff) × profitPerEuro) }
  - Compute: parallel query HELD z linked Listing (aiEstimatedValue,
    price, aiScore, dealScore) + SOLD 12m (avgROI, avgHoldDays za
    historical baseline); per held item compute capitalDeployed (buyPrice
    + buyFees), estValue, capitalEfficiency (estValue/capitalDeployed),
    profitPerEuroDeployed (profit/capitalDeployed), daysHeld,
    efficiencyLossPerDay (1.5%/mo opportunity cost); efficiencyScore =
    50% capitalEff + 30% profitPerEuro + 20% speed − trappedDuration
    penalty + 5pt historical bonus; portfolio weighted avg by capitalDeployed;
    lowEfficiencyItems (filter <40, top 8 by lowest score, recommend
    discount/bundle/cross-post); highEfficiencyOpportunities (electronics
    2.8x, phones 2.4x, gaming 2.2x); capitalReallocationPlan (low-eff
    items → high-eff categories); projectedEfficiency = (remaining × current
    + realloc × reallocatedEff) / totalCapital; efficiencyUplift = projected
    − current; expectedProfitUplift = reallocAmount × (reallocatedEff −
    currentEff) × profitPerEuro; capitalVelocityOptimization (auto-discount,
    cross-posting, bundling).
  - AI-enhanced z grounding prompt (top 40 items by lowest efficiencyScore
    + soldCount12m + avgROIHistorical + avgHoldDaysHistorical + caps +
    deterministic baseline) + anti-hallucination (tradeId MORA match-at
    topItems — skip unknown, projectedEfficiency CLAMPED [0, 10],
    efficiencyUplift [0, 10], amount [0, capitalDeployed of that item],
    expectedProfitUplift [0, 50000], enums validirana A+/A/B/C/D/F,
    string length limits — action max 200, reasoning max 400, plan max
    400, summary max 400) + 6h cache (key
    `inventory-capital-efficiency-maximizer:${JSON.stringify(heldItemIds)}`
    — invalidira ko held inventory se spremeni) + deterministic fallback
    (lowEffItems iz hevristika filter, opportunities iz fixed 3 kategorije,
    reallocation plan iz low-eff items, projected iz formula, grade iz
    score+efficiency).
  - GET+POST (handleInventoryCapitalEfficiencyMaximizer shared function —
    AI Hub runner kompatibilnost). Empty-state fallback če 0 HELD trades
    → "Ni HELD trgovin v inventarju" z aiUsed=false + empty current z 0 +
    empty perItem + empty maximization z grade F.

### Changed
- AI_ENDPOINTS.md: 362 → 365 endpoints (+3 AI: deal-profit-margin-enhancer-pro pos 93, inventory-capital-efficiency-maximizer pos 133, revenue-growth-maximizer pos 325)
- README.md: v8.00.0 → v8.01.0 badge, 362 → 365 AI endpoints, 539 → 542 API routes, ~225 → ~228 funkcij, 163+ → 166+ profit pipeline funkcij, dodan v8.01 "Kaj je novega" block (3 features z full descriptions), posodobljen Roadmap (v8.01 trenutno, 45 → 46 verzij, 135 → 138 novih funkcij), dodana 3 nova imena v Profit pipeline list, dodana Zadnje verzije v8.01.0 entry, tagline 362 → 365 AI endpointov, v7.56–v8.00 → v7.56–v8.01, v7.32-v8.00 → v7.32-v8.01, v7.50-v8.00 → v7.50-v8.01, do v8.00 → do v8.01, 362 AI → 365 AI (6 refs), 539 routes → 542 routes (3 refs)
- CHANGELOG.md: [Unreleased] Načrtovano za v8.01+ → ...za v8.02+, dodana nova [8.01.0] sekcija z vsemi 3 endpoint-i in podrobnimi opisi (response shape, anti-hallucination rules, AI cache key, deterministic fallback, example comment, razlika od podobnih obstoječih endpoint-ov). Skupno 362 AI → 365 AI (+3), 72 analytics nespremenjeno (0 new), 539 routes → 542 routes (+3), ~225 funkcij → ~228 funkcij (+3), 163+ funkcij → 166+ funkcij v profit pipeline (+3).
